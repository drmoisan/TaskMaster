    ...
    public class BayesianSerializationHelper_Tests
    {
        ...
        public void TestInitialize()
        {
            ...
            _appDataRoot = Path.Combine(
                Path.GetDirectoryName(typeof(BayesianSerializationHelper_Tests).Assembly.Location),
                "BayesianSerializationHelperTests"
            );
        ...
        public void Deserialize_WithExistingFile_ReturnsDeserializedObject()
        {
            ...
            WriteJsonFixture("fixture.json", expected);
        ...
        public async Task DeserializeAsync_WithExistingFile_ReturnsDeserializedObject()
        {
            ...
            WriteJsonFixture("fixtureAsync.json", expected);
        ...
        public async Task DeserializeAsync_WithProgressAndExistingFile_ReturnsDeserializedObject()
        {
            ...
            WriteJsonFixture("fixtureProgress.json", expected);
        ...
        public void NonPublicHelpers_ReturnExpectedDiskAndJsonSettings()
        {
            ...
            // Assert
            disk.FilePath.Should().Be(Path.Combine(_appDataRoot, "Bayesian", "scores_daily.txt"));
        ...
        public async Task SaveTextsAsync_WritesUnicodeTextAndOverwritesExistingFile()
        {
            // Arrange
            var sut = new BayesianSerializationHelper(_mockGlobals.Object);
            var folder = GetBayesianFolder();
            Directory.CreateDirectory(folder);
            var filePath = Path.Combine(folder, "notes.txt");
            ...
            // Assert
            File.ReadAllLines(filePath, Encoding.Unicode)
                .Should()
                .ContainInOrder("line one", "line two");
        }
        ...
        public async Task SaveCsvAsync_WritesCommaSeparatedText()
        {
            // Arrange
            var sut = new BayesianSerializationHelper(_mockGlobals.Object);
            Directory.CreateDirectory(GetBayesianFolder());

            // Act
            await sut.SaveCsvAsync([new[] { "A", "B" }, new[] { "1", "2" }], "matrix");

            // Assert
            var csvPath = Path.Combine(GetBayesianFolder(), "matrix.csv");
            File.ReadAllLines(csvPath, Encoding.Unicode).Should().ContainInOrder("A,B", "1,2");
        }

        [TestMethod]
        public void SerializeAndSave_WritesJsonToDisk()
        {
            // Arrange
            var sut = new BayesianSerializationHelper(_mockGlobals.Object);
            ...
            // Assert
            var filePath = Path.Combine(GetBayesianFolder(), "savedFixture.json");
            File.Exists(filePath).Should().BeTrue();
            File.ReadAllText(filePath).Should().Contain("delta");
        }
        ...
        public async Task SerializeAndSaveAsync_WritesJsonUsingProgressPath()
        {
            ...
            // Assert
            var filePath = Path.Combine(GetBayesianFolder(), "asyncSavedFixture.json");
            File.Exists(filePath).Should().BeTrue();
            File.ReadAllText(filePath).Should().Contain("epsilon");
        }
        ...
        public async Task WriteTextsAsync_AppendsEachLine()
        {
            ...
            // Assert
            File.ReadAllLines(targetFile, Encoding.Unicode)
                .Should()
        ...
        public void InternalSerializeAndSave_UsesProvidedSerializerAndDisk()
        {
            ...
            var expectedPath = disk.FilePath;
            ...
            // Assert
            File.ReadAllText(expectedPath).Should().Contain("zeta");
        }
        ...
        private void WriteJsonFixture(string fileName, SerializationFixture fixture)
        {
            Directory.CreateDirectory(GetBayesianFolder());
            var filePath = Path.Combine(GetBayesianFolder(), fileName);
            File.WriteAllText(filePath, JsonConvert.SerializeObject(fixture));
        }
