...
namespace UtilitiesCS.Test.EmailIntelligence
{
    ...
    public class ImageStripper_Tests
    {
        ...
        public void PilDecodeParts_WithNullAttachmentData_AddsInvalidImageToken()
        {
            // Arrange
            var stripper = new ImageStripper();
            var attachment = CreateAttachment(size: 10, data: null, attachmentType: OlAttachmentType.olByValue);
        ...
        public void PilDecodeParts_WithMixedContent_ConcatenatesImagesIntoSingleBitmap()
        {
            // Arrange
            var stripper = new ImageStripper();
            var first = CreateAttachmentFromBitmap(CreateBitmap(width: 2, height: 3, color: Color.Blue));
            var second = CreateAttachmentFromBitmap(CreateBitmap(width: 4, height: 3, color: Color.Green));
            var third = CreateAttachmentFromBitmap(CreateBitmap(width: 5, height: 2, color: Color.Red));

            // Act
            var (images, tokens) = stripper.PIL_decode_parts(new List<object>
            {
        ...
        private static IAttachment CreateAttachment(int size, byte[] data, OlAttachmentType attachmentType)
        {
