---
name: orchestrate-python-work
description: 'Route a Python request through budget-based orchestration: direct small-scope delivery or full feature/bug lifecycle with promotion, research, plan/execution, and review. Use when the user wants to run an end-to-end Python workflow from intake to completion.'
model: opus
allowed-tools: Read, Write, Edit, Bash, Grep, Glob, Agent, TodoWrite
---

# Python Orchestration Command

Use this command to run an end-to-end Python workflow through `python-orchestrator`.
...
3. If budget is **>3 production Python files** or **>3 test Python files**:
   - Execute full large-path lifecycle:
     ...
     5) Delegate plan creation to `python-atomic-planner` (which delegates to `python-atomic-planner`, then validates preflight via `python-atomic-executor` until `PREFLIGHT: ALL CLEAR`)
