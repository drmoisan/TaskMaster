---
description: 'Prompt for reviewing an epic documentation root folder with a delivery-first audit for a single-developer, multi-feature initiative. Derives all feature subfolders and current versions/plans from the epic root input, validates acceptance criteria against current code, reconciles plan checklists, and generates epic-audit + feature-delivery-inventory + policy-audit artifacts.'
---
...
- **Epic root folder (required):** `$ARGUMENTS`
...
Write all outputs to the epic root folder (`$ARGUMENTS`).
...
5. A **completed** `remediation-plan.<timestamp>.md` created by automatically invoking the `atomic-planner` skill
