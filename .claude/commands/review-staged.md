---
description: 'Prompt for reviewing currently staged changes before commit. Generates policy-audit.md + code-review.md in the active feature folder; if remediation is needed, produces remediation inputs and delegates remediation-plan.md creation to atomic_planner.'
---
...
Write outputs to the **active feature folder** determined by the staged-reviewer skill. If no active feature folder can be inferred, use the fallback (`docs/features/active/_staged-review/`).
...
2. `code-review.<timestamp>.md`
   - A staged-diff-based review emphasizing repo compliance and code quality best practices.
