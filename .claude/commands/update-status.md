...
- **Allow GitHub mutations (optional):** `AllowGitHubMutations=true|false`
  - Default behavior is **false** (no remote changes).
  - If true, the skill may use `gh` CLI to update remote issue state/content only if authenticated and safe.
...
Write outputs to the epic root folder.
...
- Update relevant `plan.<timestamp>.md` files:
  - check off `[ ]` to `[x]` only when evidence supports it
