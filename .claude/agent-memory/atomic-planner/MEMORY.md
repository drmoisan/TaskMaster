...
- [#511 R1](project_511_r1_preflight_delta_seams.md) — mid-cycle evidence deletion; Start-Process for 20-min runs; per-class coverage noise
...
- [QfcItemController capacity squeeze](project_qfcitemcontroller_test_capacity_squeeze.md) — ~471 spare lines across 4 test files, `.csproj` edits barred
- [#494](project_494_threshold_reconciliation_plan_seams.md) — coverage runner throws before post-processing; reported floor ≠ hook-Blocking
...
- [#464 R3/R4](project_464_efc_controller_plan_seams.md) — additive-only growth is a ceiling not a shrink; phase-N counts survive earlier deletions
- [#677 R1–R8](project_677_keyboard_focus_leak_plan_seams.md) — ctor param REJECTED (5 reflection-arity tests); typed harness for compile-red
- [#635](project_635_reflective_caller_audit_plan_seams.md) — evidence-only audit inflates its own sweep; spec said six sites, tree has eight
- [#440 R1–R4](project_440_breadcrumb_left_arrow_plan_seams.md) — deletion voids a changed-line gate; `(Rebuild target(s))`; `.dotnet-sdk` IS gitignored
...
- [#637 R2–R5](project_637_selectrow_rooted_path_plan_seams.md) — `docs/features/active` hits 121 sibling files; blanket `-F` breaks every regex
- [#469 R1–R3](project_469_comment_accuracy_plan_seams.md) — defect-number SWAP voids token gates; `- [x] AC1` is a prefix of `AC10`
- [#644](project_644_ac16_referral_revision_seams.md) — AC's named instrument prints no figure; check-off task → referral pinning UNCHECKED
...
- [Validator task-ID sequential constraint](plan-validator-task-id-sequential-constraint.md) — insertion forces a full renumber
- [Planner may lack the MCP validator](project_planner_mcp_validator_not_in_tool_surface.md) — report VALIDATOR NOT RUN; never claim a pass
- [Fenced `#` comments look like headings](plan-fenced-powershell-comments-look-like-headings.md) — indent column-0 `#` inside fences
- [One AC per check-off task](feedback_ac_checkoff_one_per_task.md) — preflight rejects batched AC check-offs
- [Terminal-phase planner traps](terminal-phase-planner-traps.md) — unowned follow-ups; artifacts after the clean-tree commit task
- [Never plan a mid-plan halt on MCP availability](never-plan-a-mid-plan-halt-on-mcp-availability.md) — Phase 0 probe, then continue
- [Thread granted discharges through consumers](thread-granted-discharges-through-consumers.md) — softening one task strands its consumer
...
- [Acceptance edits must be false-before/true-after](acceptance-edits-must-be-false-before-true-after.md) — a clause true at branch head is a no-op
...
- [Literal-call clauses block file-size tightening](literal-call-clauses-block-file-size-tightening.md) — 2+ pinned calls near a 500-line file
- [Enumeration variable must match its consumer](enumeration-variable-must-match-consumer.md) — `$kept` vs `@assemblies` = zero-assembly run
...
- [Stale build output is not evidence of existence](stale-build-output-is-not-evidence-of-existence.md) — use `git ls-files`, not `obj/`
...
- [Phase 0 toolchain bootstrap](project_csharp_phase0_toolchain_bootstrap.md) — csharpier works once the SDK is bootstrapped; NuGet restore mandatory
...
- ["Formatted N files" is a processed count](csharpier-formatted-n-is-processed-count.md) — define rewritten-count via before/after SHA-256
- [Repo-wide csharpier format breaks zero-diff ACs](csharpier-repowide-format-breaks-zero-diff-acs.md) — scope the mutating pass to the path list
...
- [PowerShell gate observables](powershell-gate-observables.md) — no Invoke-Pester exit code; aggregate-only `CoveragePercent`
...
- [net48 / nullable context mismatch](project_nullable_context_mismatch_prod_vs_test.md) — check `#nullable enable` and the test csproj `<LangVersion>`
...
- [Deletion-adjusted no-regression gate](deletion-adjusted-coverage-no-regression-gate.md) — gate on covered/valid counters, not the rate
- [#489 PartN reroute](project_489_partn_reroute_amendment_seams.md) — verify parent `partial` first; amendment notes shift all AC line citations
...
- [#493 UiThread dispatcher](project_493_uithread_dispatcher_plan_seams.md) — stage the two `<Compile Include>` lines for a REAL red build
- [#442 QuickFiler metrics](project_442_quickfiler_metrics_plan_seams.md) — commented-out code defeats zero-hit greps; seam before red tests
- [#468 QfcCollectionController](project_468_qfc_collection_controller_plan_seams.md) — a sign-defect seam must land carrying the defect
...
- [Enumerate condition outcomes first](enumerate-condition-outcomes-before-case-list.md) — 2 outcomes per condition in every `||`/`&&`
- [#441 Cobertura arithmetic](project_441_cobertura_arithmetic_plan_seams.md) — StrictMode throws on any missing fixture attribute
...
- [Re-scoping after a sibling landed the fix](plan-rescope-after-sibling-landed-the-fix.md) — split the contiguous TAIL so citations survive
...
- [#445 keyboard-action](project_445_keyboard_action_plan_seams.md) — resolve WS at execution time; scope epic-child gates to owned classes
...
- [#438 search-focus](project_438_search_focus_plan_seams.md) — an additive interface overload broke 7 test files
...
- [#328 store exclusion](project_328_store_exclusion_seams.md) — new test `.cs` need csproj wiring; four inclusion surfaces lockstep
- [#295 WinForms STA exemptions](project_winforms_sta_refinement_exemption_rule.md) — keep dialog/Form/launcher exemptions only
- [#295 STA control-identity pattern](project_sta_last_resort_control_identity_pattern.md) — companion interface; never construct a Form
...
- [Dispatcher repro hang trap](dispatcher-repro-hang-trap.md) — use an owned pumping STA thread, not `Dispatcher.CurrentDispatcher`
