...
**Update (2026-08-06, #424 v1.2):** the measured TaskMaster repo-wide baseline is 70.19% line / 58.30% branch — below every documented floor before any change. Orchestrator-ratified handling: the 80% floor applies to the *testable denominator* after the CLAUDE.md § UT2 COM/VSTO/WinForms exemptions; the raw uninstrumented repo-wide figure is pre-existing merge-base debt and is reported non-blocking on bug-fix plans, while changed-line no-regression and new/changed >=90% remain fully blocking. Cite this precedent (plan Decisions item 13 in the #424 plan) rather than hard-gating a fix branch on the raw repo-wide number.
