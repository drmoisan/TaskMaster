...
`scan_folders` genuinely narrows `run_poshqc_analyze` (`["tests/scripts/vscode"]` -> ok:true; adding `scripts/vscode` -> 16 issues), so scoping cannot rescue a folder that already carries findings.
