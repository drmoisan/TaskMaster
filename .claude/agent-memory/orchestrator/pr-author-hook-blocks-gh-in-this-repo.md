...
**SUPERSEDED 2026-07-06 (later same day):** `.claude/lib/orchestrator-state/OrchestratorState.psm1` was updated to add a portable PowerShell fallback. `Invoke-OrchestratorStatePreflight` now probes `Test-PythonOrchestratorValidatorAvailable`; when `scripts.dev_tools` is NOT importable (this repo), it routes to `Test-OrchestratorStatePrCreationReadiness` (pure PowerShell, no python). So autonomous PR creation IS possible here now — the preflight passes when the checkpoint is well-formed. It fails closed on a bad checkpoint (e.g. an invalid step-status enum value). Always run the preflight (`Import-Module .../OrchestratorState.psm1; Invoke-OrchestratorStatePreflight -CheckpointPath ...`) and fix the reported error rather than assuming a hard block. Enum for step*_status: `{not-applicable, pending, delegated, verified, blocked}`.

--- ORIGINAL NOTE (no longer true, kept for history) ---
The registered PreToolUse hook `.claude/hooks/enforce-pr-author-skill.ps1` runs a PR-creation preflight whose default `$Invoker` is `python -m scripts.dev_tools.validate_orchestration_artifacts orchestrator-state <checkpoint> --require-pr-creation-ready`. That python package **did not exist** in this repo, so the invoker exited 1 and the hook denied every `gh pr create`. This was fixed by the portable fallback described above.
