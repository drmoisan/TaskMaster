...
**How to apply:**
- **Host-token adjudication precedent:** 488's 19 committed TRX embed `c:\users\<account>\...` in `storage`/`codeBase` and `<host>` in `runUser` while name/user/computerName were placeholder-sanitized. Adjudicated **Non-blocking** because identical tokens already exist in merged sibling evidence (501, 608, 439) on the integration branch — the branch neither introduces nor can cure the class. Sibling 489 achieved FULL sanitization, so full is the current standard; check `runUser` and lowercase `storage` paths specifically, sanitizers miss them. Fan-in owes one sed pass over `evidence/**/*.trx` + a repo-wide cleanup entry.
