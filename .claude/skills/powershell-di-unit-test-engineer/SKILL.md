...
description: 'Plans and implements minimal DI seams and Pester v5 unit tests with correct mocking patterns (especially for external executables), while enforcing strict scope, PSScriptAnalyzer hygiene, and zero-regression quality gates. Use when the primary goal is testability improvement or fixing failing Pester tests through targeted DI refactoring on one production file and its corresponding tests.'
...
You must run the repo toolchain via Bash.
...
When a remediation plan is needed without implementation, use `Agent(subagent_type="general-purpose", prompt="...")`:

- **Remediation plan only (no edits)**: Enumerate failing tests, root causes, minimal DI seams, mock strategy, and exact files to change. Do not edit code.
- **Implement approved plan with strict gates**: Implement ONLY the approved plan. Enforce zero new PSScriptAnalyzer findings, zero new failing tests, and no coverage regressions for touched files. Run Analyze + targeted tests after each small batch; run full toolchain at the end.
- **Post-change QA gate**: Run the full toolchain and report deltas: analyzer findings, failing tests, and per-file coverage for touched files. If any regression exists, revert/fix before proceeding.
...
Assume VS Code Test Explorer may run with different PATH, working directory, profile state, and PowerShell host/session. Therefore:
