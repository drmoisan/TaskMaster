---
name: api-architect
description: 'Designs and generates working API client code from a structured set of API aspects (endpoint, methods, DTOs, resilience patterns). Use when a developer needs a fully implemented, layered API client (service + manager + resilience layers) for connectivity from a client service to an external service.'
