...
description: 'Deterministic promotion workflow from potential feature/bug entry to issue, branch, active feature folder, and downstream spec/research handoffs. Prefer VS Code extension command execution when available; use scripts only as fallback.'
...
When the agent has access to the VS Code extension tool surface, execute the lifecycle through the contributed extension commands first.

Canonical extension command invocations (via Bash with `code --command` or the VS Code CLI equivalent):
...
Fallback rule:
- Use direct script/CLI commands only when VS Code extension commands are unavailable.
...
- feature: VS Code command `drmCopilotExtension.newPotentialEntry -ShortName ${short-name}`
- bug: VS Code command `drmCopilotExtension.newPotentialBugEntry --short-name ${short-name}`

2) Promote potential doc:
- VS Code command `drmCopilotExtension.potentialToIssue --potential-path ${relativeFile} --promotion-type ${promotion-type} --work-mode ${work-mode}`

3) Create branch:
- `${promotion-type}/${short-name}-${issue-num}`

4) Create active feature folder:
- VS Code command `drmCopilotExtension.newActiveFeatureFolder --feature-name ${long-name} --type ${promotion-type} --issue-number ${issue-num} --work-mode ${work-mode}`
...
- VS Code command `drmCopilotExtension.potentialToIssue --potential-path ${relativeFile} --promotion-type ${promotion-type} --work-mode minor-audit`

2) Create branch:
- `${promotion-type}/${short-name}-${issue-num}`

3) Create active feature folder with short-path flag:
- VS Code command `drmCopilotExtension.newActiveFeatureFolder --feature-name ${long-name} --type ${promotion-type} --issue-number ${issue-num} --work-mode minor-audit`
...
4) Delegate minimal-audit plan creation to `atomic-planner` with directive:
...
5) Require preflight validation via `atomic-executor` until:
- `PREFLIGHT: ALL CLEAR`
