...
description: 'Budget-first routing contract for C# work: estimate production-file scope, choose small vs large path, enforce orchestration-first routing for larger changes. Use when starting any C# implementation request to determine whether to route to direct or orchestrated flow.'
...
- invoke promotion/folder lifecycle steps per `feature-promotion-lifecycle` when available; use script/CLI fallback only when direct extension command execution is unavailable,
- promote potential item to GitHub issue with `--work-mode minor-audit`,
- create active feature folder with `--work-mode minor-audit`,
- delegate minimal-audit plan creation to `atomic-planner` with `DIRECTIVE: MINIMAL-AUDIT PLAN REQUIRED`,
- require `atomic-executor` preflight until `PREFLIGHT: ALL CLEAR`,
- execute Phase 0 only via atomic-executor before branching,
...
- Return explicit routing instruction to invoke `csharp-orchestrator` (or `/orchestrate-csharp-work`).
