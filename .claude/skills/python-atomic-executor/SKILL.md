...
4) Any applicable language-specific policies (Python, etc.)
...
3) Provide a *plan delta* (exact new/modified task(s) that `python-atomic-planner` should add), preserving the plan's ID conventions.
4) Ask the user to run `python-atomic-planner` to produce the corrected plan (or to explicitly approve the delta).
...
1) `.claude/skills/policy-compliance-order/SKILL.md`
2) `.claude/skills/general-code-change-policy/SKILL.md`
3) `.claude/skills/python-code-change-policy/SKILL.md` (if present)
4) `.claude/skills/general-unit-test-policy/SKILL.md`
5) `.claude/skills/python-unit-test-policy/SKILL.md` (if present)
6) `.claude/skills/python-suppressions-policy/SKILL.md` (if present)
...
4) Test (`poetry run pytest --cov=src --cov-report=term-missing` or repo task equivalent)
