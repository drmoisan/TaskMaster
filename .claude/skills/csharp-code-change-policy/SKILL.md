...
description: 'C#-specific code change rules layered on top of the general code change policy. Use for all C# source, test, and build-configuration files (*.cs, *.csproj, *.props, *.targets).'
user-invocable: false
---

# C# Code Change Policy

This policy **extends** `general-code-change-policy` and applies to all **C# source, test, and build-configuration files** (`*.cs`, `*.csproj`, `*.props`, `*.targets`) in this repo.

You must:
- Apply **all** rules in the general code change policy.
- Apply **all** C#-specific rules in this file.
- Apply the unit test policies (`general-unit-test-policy` and `csharp-unit-test-policy`) for any work involving tests.
...
> **Testing tools and behavior are defined in the unit test policies.** Do not define test behavior here; instead, obey `general-unit-test-policy` and `csharp-unit-test-policy`.
...
1. Keep files focused on one responsibility area. Keep file size under the repo limit in `general-code-change-policy`.
