...
description: 'Orchestrates end-to-end PowerShell feature and bug delivery by estimating change budget and routing small changes through a short-path (promotion, minimal-audit plan, execution, reduced audit) and larger efforts through the full lifecycle (scope, research, spec, atomic planning, atomic execution, feature review) until completion. Use when a PowerShell request requires full workflow coordination across specialist agents.'
...
S4.1 Delegate to `powershell-atomic-executor` via Agent tool:

> Execute the approved plan in `${feature-folder}` with strict phase scoping: run Phase 0 only and stop. Return execution summary and updated checklist state. Do not execute Phase 1 or Phase 2.
...
Delegate to `powershell-typed-engineer` via Agent tool:

> Estimate and confirm scope (1-2 production PowerShell files + corresponding tests). If confirmed, execute the constrained short-path implementation phase from the approved minimal-audit plan and return analyzer/test/coverage deltas. Feature folder: `${feature-folder}`. Approved plan: `${plan-path}`.
...
S7.1 Delegate to `powershell-atomic-executor` via Agent tool:
...
1.5 Delegate to a general-purpose PRD agent via Agent tool to fill generated form details only, preserving headings/template structure.
...
3.1 Delegate to `task-researcher` via Agent tool:
- Use `.claude/commands/` research prompt with the issue path context to generate implementation research artifacts.
- Pass `${feature-folder}/issue.md` as primary context.

3.2 After research exists, delegate to a general-purpose PRD agent via Agent tool:
- Pass links to issue and newly created research.
- Enforce detailed technical specification completion.
- Preserve headings and thoroughly complete technical details.
...
Delegate to `powershell-atomic-planner` via Agent tool with the following context package:
...
- required `${plan-path}` for in-place updates

Hard enforcement for Step 4:
- The planning route MUST be `powershell-atomic-planner -> atomic-planner -> powershell-atomic-executor` for preflight validation.
...
Delegate to `powershell-atomic-executor` via Agent tool with:
- `${feature-folder}`
...
Use `Agent(subagent_type="general-purpose", prompt="...")` for all specialist handoffs. Key delegation targets:

- **powershell-atomic-planner**: planning and preflight all-clear loops
- **powershell-atomic-executor**: plan execution, preflight validation, and final QA
- **powershell-typed-engineer**: small-scope direct implementation with quality gates
- **feature-reviewer**: post-implementation policy/code/feature audit and remediation trigger
- **task-researcher**: implementation research artifact generation
