...
description: 'Design and implement small, highly testable, pythonic modules and classes with strong typing (Pyright), repo-standard formatting/linting (Black+Ruff), and deterministic Pytest coverage while enforcing strict scope and zero-regression gates. Use when delivering a Python feature or bug fix covering 1-3 production files with full baseline capture, design review, implementation, and QA closure.'
...
2) `.claude/skills/python-code-change-policy/SKILL.md` (if present)
3) `.claude/skills/general-unit-test-policy/SKILL.md`
4) `.claude/skills/python-unit-test-policy/SKILL.md` (if present)
...
- If no override is provided, the 3/3 limit applies; if an override is requested, confirm compliance before Phase C.
...
If no plan is provided, delegate the creation of a plan to `python-atomic-planner` via the Agent tool:

```
Agent(subagent_type="general-purpose", prompt="Create a plan ONLY: proposed module/class design, typed interfaces, DI seams, test strategy (Pytest), and exact files to change. Do not edit code.")
```
...
When delegating sub-tasks, use `Agent(subagent_type="general-purpose", prompt="...")`:

- **Architecture + testability plan only (no edits)**: Delegate to `python-atomic-planner` with prompt asking for proposed module/class design, typed interfaces, DI seams, test strategy (Pytest), and exact files to change without editing code.
- **Post-change QA gate (format + lint + type + tests + coverage)**: Delegate to a general-purpose agent to run the full toolchain and report deltas: Ruff findings, Pyright diagnostics, failing tests, and per-file coverage for touched files.
