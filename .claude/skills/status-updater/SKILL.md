---
name: status-updater
description: 'Synchronizes status across epic docs, feature docs, atomic plans, and optionally GitHub Issues. Checks off delivered-but-unchecked plan items, reconciles issue/doc status, and documents acceptance-criteria evidence in authoritative requirement files. Use when epic or feature delivery state needs to be synchronized with the actual codebase evidence.'
...
- Aligning **plans vs. delivered work vs. documentation vs. issues**
...
- The run is driven by the provided EpicRootFolder ("<EPIC_FOLDER>").
...
Unless `AllowGitHubMutations` is literally `true` (case-insensitive) AND `gh` CLI auth is confirmed,
...
## B) Documentation vs. Issues synchronization
