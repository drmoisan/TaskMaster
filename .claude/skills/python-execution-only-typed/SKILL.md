...
description: 'Implement small, highly testable, pythonic modules and classes with strong typing (Pyright), repo-standard formatting/linting (Black+Ruff), and deterministic Pytest coverage while enforcing strict scope and zero-regression gates. Use when an approved plan already exists and only execution is needed, with a higher file budget (up to 30 production files) for larger execution-only tasks.'
...
- Override by specifying 'budget: prod=<N>, test=<M>' in the user prompt before Phase B begins.
- If no override is provided, the 30/30 limit applies; if an override is requested, confirm compliance before Phase B.
...
When an architecture or testability plan is needed before execution begins, delegate to `python-atomic-planner` via the Agent tool:
