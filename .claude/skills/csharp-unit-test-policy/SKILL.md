...
description: 'C#-specific unit test rules layered on top of the general unit test policy. Use for all C# unit tests (*.cs test files).'
user-invocable: false
---

# C# Unit Test Policy

This policy **extends** `general-unit-test-policy` and applies to all C# unit tests in this repo.
...
The loop behavior (restart rules, must-pass requirements, and audit expectations) is defined by `general-code-change-policy` and is intentionally not repeated here.

This file is intentionally limited to C#-specific framework/library/tool selection. Cross-language testing principles and policy requirements are defined in `general-unit-test-policy` and `general-code-change-policy`.
