...
Public Class QuickFileControllerOrig

    Private OlApp_hWnd As IntPtr
    Private lFormHandle As IntPtr
    ...
    Public Sub New(AppGlobals As IApplicationGlobals,
        ...
        '_heightFormMax = ScreenHeight()
        Dim _screen = Screen.FromControl(_viewer)
        Height_UserForm_Max = _screen.WorkingArea.Height
        ...
        '_intEmailsPerIteration = CInt(Round((_heightPanelMainMax / (frmHt + frmSp)), 0))
    ...
    Public Sub SetAPIOptions()
        ...
        'CHANGE Form now grabbed directly from windows forms
        '_lFormHandle = FindWindow("ThunderDFrame", _viewer.Text)
        '_olAppHWnd = FindWindow(olAppCLSN, vbNullString) 'Grabs handle on everything including vba
        'Dim OlApp_hWnd2 As LongPtr
        'OlApp_hWnd2 = GetAncestor(_lFormHandle, GA_ROOTOWNER)

        OlApp_hWnd = PInvoke.GetAncestor(lFormHandle, UI.WindowsAndMessaging.GET_ANCESTOR_FLAGS.GA_PARENT)
        '_olAppHWnd = GetAncestor(_lFormHandle, GA_PARENT)


        'Debug.Print "_lFormHandle " & _lFormHandle
        'Debug.Print "_olAppHWnd " & _olAppHWnd
        ...
        'PInvoke.GetWindowLongPtr(_lFormHandle, PInvoke.WindowLongParam.GWL_STYLE)
        ...
        'the attributes of the specified window , given as _lFormHandle,
        'GWL_STYLE = New windows style, and our Newly defined style = lStyle
        'SetWindowLongPtr(_lFormHandle, GWL_STYLE, lStyle)

        'Remove >'&LT; if you want to show form Maximised
        'ShowWindow _lFormHandle, SW_SHOWMAXIMIZED 'Shows Form Maximized
        ...
        'PInvoke.DrawMenuBar(_lFormHandle)
        'PInvoke.ShowWindow(_lFormHandle, UI.WindowsAndMessaging.SHOW_WINDOW_CMD.SW_SHOWMAXIMIZED)
    ...
    Public Sub Init(colEmails As Collection)
        ...
        For Each objItem In colEmails
            If TypeOf objItem Is MailItem Then
                ...
                QF = New QfcControllerOrig(Mail, colCtrls, intUniqueItemCounter, BoolRemoteMouseApp, Caller:=Me, AppGlobals:=_globals, hwnd:=lFormHandle, InitTypeE:=InitType)
    ...
    Optional blWideView As Boolean = False)
        ...
        If ((intItemNumber * (frmHt + frmSp)) + frmSp) > _viewer.PanelMain.Height Then      'Was _heightPanelMainMax but I replaced with Me.Height
    ...
    Private Sub RemoveControls()
        ...
        '_viewer.PanelMain.ScrollHeight = _heightPanelMainMax
    ...
    Public Sub AddEmailControlGroup(Optional objItem As Object = Nothing, Optional posInsert As Integer = 0, Optional blGroupConversation As Boolean = True, Optional ConvCt As Integer = 0, Optional varList As Object = Nothing, Optional blChild As Boolean = False)
        ...
        If TypeOf objItem Is MailItem Then
            ...
            QF = New QfcControllerOrig(Mail, colCtrls, posInsert, BoolRemoteMouseApp, Me, _globals)
    ...
    Public Sub ConvToggle_Group(selItems As Collection, intOrigPosition As Integer)
        ...
        Dim i As Integer
        Dim QF As QfcControllerOrig
        Dim QF_Orig As QfcControllerOrig
    ...
    Friend Sub RemoveSpecificControlGroup(intPosition As Integer)
        ...
        If intPosition < intItemCount Then
            ...
            Next i
            '_viewer.PanelMain.ScrollHeight = max(_viewer.PanelMain.ScrollHeight - frmHt - frmSp, _heightPanelMainMax)
        End If
    ...
    '        _viewer.PanelMain.ScrollTop = _lngPanelMainSCTop
    '    ElseIf (frmSp + lngItemBottom) > (_lngPanelMainSCTop + _viewer.PanelMain.Height) Then
    '        DiffY = frmSp + lngItemBottom - (_lngPanelMainSCTop + _viewer.PanelMain.Height)
