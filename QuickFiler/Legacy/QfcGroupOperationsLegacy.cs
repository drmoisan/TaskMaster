...
namespace QuickFiler.Legacy
{
    ...
    internal class QfcGroupOperationsLegacy : IAcceleratorCallbacks, IQfcControllerCallbacks
    {
        ...
        internal void LoadGroupOfCtrls(
            ref List<Control> colCtrls,
            int intItemNumber,
            int intPosition = 0,
            bool blGroupConversation = true,
            bool blWideView = false
        )
        {
            long lngTopOff;
            bool blDebug = false;
            ...
            if (
                intItemNumber * (QfcConstants.Panel.Height + QfcConstants.FrmSp)
                    + QfcConstants.FrmSp
                > _viewer.L1v1L2_PanelMain.Height
            ) // Was _heightPanelMainMax but I replaced with Me.Height
            {
            ...
            Pnl.Top =
                (QfcConstants.FrmSp + QfcConstants.Panel.Height) * (intPosition - 1)
                + QfcConstants.FrmSp
                + QfcConstants.ScaledInt(16);
            ...
            if (_initType.HasFlag(QfEnums.InitTypeEnum.Sort))
            {
                ...
                chbxDelFlow.Left =
                    chbxSaveMail.Left - chbxDelFlow.Width - QfcConstants.ScaledInt(1);
                ...
                chbxSaveAttach.Left =
                    chbxDelFlow.Left - chbxSaveAttach.Width - QfcConstants.ScaledInt(1);
                ...
                chbxGPConv.Left =
                    chbxSaveAttach.Left - chbxGPConv.Width - QfcConstants.ScaledInt(1);
        ...
        private void AssignDimensions(
            ...
            QfcConstants.ConstantGroup constantGroup
        )
        {
        ...
        internal void MoveDownControlGroups(int intPosition, int intMoves)
        {
            ...
            for (i = _listQFClass.Count - 1; i >= loopTo; i -= 1)
            {
                ...
                ctlFrame.Top =
                    ctlFrame.Top + intMoves * (QfcConstants.Panel.Height + QfcConstants.FrmSp);
            }
        ...
        public void AddEmailControlGroup(
            object objItem,
            int insertAtIndex = 0,
            bool blGroupConversation = true,
            int ConvCt = 0,
        ...
        public void RemoveSpecificControlGroup(int index)
        {
            ...
            if (blDebug)
            {
                ...
                for (i = 0; i <= loopTo; i++)
                {
                    ...
                    {
                        Debug.WriteLine(
                            i + "  " + intDeletedMyPos + "  " + strDeletedDte + "  " + strDeletedSub
                        );
                    }
                    else
                    {
                        ...
                        Debug.WriteLine(
                            i
                                ...
                                + QF.Position
                                + "  "
                                + QF.Mail.SentOn.ToString(@"MM\\DD\\YY HH:MM")
                                + "  "
                                + QF.Mail.Subject
                        );
                    }
        ...
        public void ConvToggle_Group(List<MailItem> selItems, int indexOriginal)
        {
            ...
            if (blDebug)
            {
                ...
                {
                    Debug.WriteLine(
                        $"_listQFClass({i++})   MyPosition {qfTemp.Position}     {qfTemp.Mail.Subject}"
                    );
                }
            }
        ...
        public void ConvToggle_UnGroup(
            List<MailItem> selItems,
            int qfIndex,
            int convCt,
            object varList
        )
        ...
        {
            _parent.ExplConvView_ToggleOn();
        }

        public void ExplConvView_ToggleOff()
        {
            _parent.ExplConvView_ToggleOff();
        }

        public bool BlShowInConversations
        {
            get => _parent.BlShowInConversations;
            set => _parent.BlShowInConversations = value;
        }
            ...
            if (_listQFClass is not null)
            {
                foreach (QfcController QF in _listQFClass)
                {
                    if (QF.BlHasChild)
                    {
                        ...
                        QF.ItemPanel.Width = (int)(
                            QfcConstants.Width_frm + intDiffx - QfcConstants.Panel.Height
                        );
            ...
            if (_intActiveSelection != 0)
            {
                ...
                if (QF.BlExpanded)
                {
                    MoveDownPix(
                        ...
                        (int)Math.Round(QF.ItemPanel.Height * -0.5d)
                    );
                    QF.ExpandCtrls1();
                    blExpanded = true;
                }
                ...
                if (col is not null)
                {
                    ...
                    for (i = 1; i <= loopTo; i++)
                    {
                        ...
                        if (objItemInCol is MailItem)
                        {
                            ...
                            if (
                                (objMailInCol.ConversationID ?? "")
                                == (objMail.ConversationID ?? "")
                            )
        ...
        {
            int idx = _listQFClass.FindIndex(
                startIndex: 0,
                count: 1,
                match: qf => qf.Mail == objMail
            );
            return idx;
        }
        ...
        internal bool ReadyForMove
        {
            get
            {
                ...
                foreach (QfcController QF in _listQFClass)
                {
                    ...
                    if (
                        (QF.FolderCbo.SelectedItem is null)
                        || headers.Contains(QF.FolderCbo.SelectedItem as string)
                    )
                    {
                        ...
                        strNotifications =
                            ...
                            + QF.Position
                            + "  "
                            + QF.Mail.SentOn.ToString("MM/dd/yyyy")
                            + "  "
                            + QF.Mail.Subject
                            + System.Environment.NewLine;
                    }
        ...
        internal string[] GetMoveDiagnostics(
            string durationText,
            ...
            for (k = 1; k <= loopTo; k++)
            {
                ...
                if (
                    infoMail.Init_wMail(
                        QF.Mail,
                        OlEndTime: OlEndTime,
                        lngDurationSec: (int)Math.Round(Duration)
                    )
                )
                {
                    ...
                    {
                        OlAppointment.Body =
                            OlAppointment.Body + System.Environment.NewLine + infoMail.ToString;
                        OlAppointment.Save();
                    }
                }
