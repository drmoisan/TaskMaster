...
- Toolchain commands to run (format → lint → type-check → test), in one uninterrupted final pass and restarted from formatting after any failure or file change:
  1. Derive the exact issue-#400 C# path set from the live merge base plus untracked `QuickFiler/**/*.cs`, `QuickFiler.Test/**/*.cs`, `UtilitiesCS/**/*.cs`, and `UtilitiesCS.Test/**/*.cs` files; exclude the unrelated committed `UtilitiesCS/EmailIntelligence/ClassifierGroups/SpamBayes/SpamBayes.Actions.cs`; sort the exact set with `StringComparer.OrdinalIgnoreCase`; require exactly 62 paths and LF-joined SHA-256 `E2439D9F8A28D97A05EA3EEFB3201587904CC784FCB9EF7200632F6BEED3EBCD`; then run `csharpier format @authorized` followed by `csharpier check @authorized`.
