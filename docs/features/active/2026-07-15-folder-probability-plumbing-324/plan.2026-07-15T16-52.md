...
- [x] [P0-T1] Read the policy files in the `policy-compliance-order` sequence (`CLAUDE.md`, `.claude/rules/general-code-change.md`, `.claude/rules/general-unit-test.md`, `.claude/rules/quality-tiers.md`, `.claude/rules/csharp.md` if present) and record the read
...
- [x] [P0-T2] Capture baseline formatting state by running `dotnet tool run csharpier .` on the clean worktree
...
- [x] [P0-T3] Capture baseline analyzer build by running the analyzer msbuild command
...
- [x] [P0-T4] Capture baseline nullable/type build by running the nullable msbuild command
  - File: `docs/features/active/2026-07-15-folder-probability-plumbing-324/evidence/baseline/baseline-nullable-build.md`
  - Acceptance: artifact records `Timestamp:`, `Command:`, `EXIT_CODE:`, `Output Summary:` (warning/error counts).
- [x] [P0-T5] Capture baseline test + coverage by running `vstest.console.exe UtilitiesCS.Test\bin\Debug\UtilitiesCS.Test.dll /EnableCodeCoverage`
...
- [x] [P1-T1] Create the `FolderScore` value type
...
- [x] [P1-T2] Wire `FolderScore.cs` into the production project
...
- [x] [P1-T3] Extract a shared ordered-scores helper in `FolderScorer` and route both existing name-only methods through it
...
- [x] [P1-T4] Add the scored projection methods on `FolderScorer`
...
- [x] [P1-T5] Create the scorer regression/characterization tests (golden baseline + ordering parity)
...
- [x] [P1-T6] Create the scored-projection and edge-case tests
...
- [x] [P1-T7] Wire the Layer-1 test files into the test project
...
- [x] [P2-T1] Create the `FolderRow` row model and kind enum
...
- [x] [P2-T2] Wire `FolderRow.cs` into the production project
...
- [x] [P2-T3] Add the `FolderRowArray` property to `FolderPredictor` mirroring `FolderArray`
...
- [x] [P2-T4] Add the `FindFolderRows(...)` method to `FolderPredictor` mirroring `FindFolder`
...
- [x] [P2-T5] Create the row-model tests using the existing mocked-Outlook harness
...
- [x] [P2-T6] Wire `FolderRowTests.cs` into the test project
...
- [x] [P3-T1] Record a fail-before exception dossier for the additive contract
...
- [x] [P3-T2] Document downstream contract sufficiency for 9002 and 9003
...
- [x] [P4-T1] Run formatting: `dotnet tool run csharpier .`
...
- [x] [P4-T2] Run analyzer build: `msbuild TaskMaster.sln /t:Build /p:Configuration=Debug /p:Platform="Any CPU" /p:EnableNETAnalyzers=true /p:EnforceCodeStyleInBuild=true`
...
- [x] [P4-T3] Run nullable/type build: `msbuild TaskMaster.sln /t:Build /p:Configuration=Debug /p:Platform="Any CPU" /p:Nullable=enable /p:TreatWarningsAsErrors=true`
...
- [x] [P4-T4] Run tests with coverage: `vstest.console.exe UtilitiesCS.Test\bin\Debug\UtilitiesCS.Test.dll /EnableCodeCoverage`
...
- [x] [P4-T5] Verify coverage delta and thresholds against the stricter regime
...
- [x] [P4-T6] Verify acceptance-criteria checkoff against evidence
