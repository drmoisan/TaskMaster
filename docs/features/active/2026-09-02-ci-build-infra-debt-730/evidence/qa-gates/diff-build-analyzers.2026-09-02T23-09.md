# QA Gate — Diff Content Check: `.github/workflows/_build-analyzers.yml`
...
Command: `git diff --cached origin/main -- .github/workflows/_build-analyzers.yml`
...
Cross-check: `git diff --cached origin/main --numstat -- .github/workflows/_build-analyzers.yml` independently reports `16	0` for this file.

Encoding note: the comparison was run with `[Console]::OutputEncoding` set to UTF-8. An initial run under the host's default console encoding reported one spurious mismatch on the comment line ending in an em dash (`... packages.config-style restores —`); that was a console transcoding artifact in the captured `git diff` text, not a content difference. Under UTF-8 output encoding all 16 lines match exactly.
