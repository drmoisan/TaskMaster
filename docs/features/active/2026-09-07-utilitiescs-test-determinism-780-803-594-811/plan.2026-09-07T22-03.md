...
- **Owner:** drmoisan
- **Last Updated:** 2026-09-08T09-06
- **Status:** Approved for execution. Executor preflight returned PREFLIGHT: ALL CLEAR on round 3, after the round-1 delta (nine defects) and the round-2 delta (four defects) were applied. The MCP plan validator returns ok with no warnings in either channel. Re-anchored at 2026-09-08T09-06 after origin/main was merged into this branch as merge commit bb1c7d4b60f7b782227956f36859314d5c47bb03. Two classes of fact moved and nothing else did: the anchor commit itself, and `UtilitiesCS.Test/UtilitiesCS.Test.csproj`, which gained three `<Compile Include>` items from the already-merged items #809 and #812. Those three items shift the project file from 982 to 985 lines and move two of its three cited item positions. Every other write-set citation and line count was re-measured against the merged tree and is unchanged, including all sixteen `.cs` line counts and `TimeOutTask.cs`.
- **Version:** 1.3
- **Work Mode:** full-bug
- **Base commit:** bb1c7d4b60f7b782227956f36859314d5c47bb03 (the origin/main merge commit on this branch; the pre-merge anchor was 04a54e681bd21e841e124c016df30672ee701b75, which is no longer a valid diff anchor because main advanced 31 commits behind this branch)
...
20. `UtilitiesCS.Test/UtilitiesCS.Test.csproj` (985; Compile items only; the 500-line cap does not reach `*.csproj`)
...
- **D13. New test files must be registered.** `UtilitiesCS.Test.csproj` is a legacy non-SDK project with explicit `<Compile Include>` items (in the merged base tree the `DfDeedle` family sits at lines 189-192, `TestHelpers\` at 74-77, and `OutlookObjects\Table\OlTableExtensions_Tests.cs` at 541). An unregistered test file still builds and is silently absent from the assembly. Every task that creates a test file is immediately followed by the task that registers it, and P8-T1 carries a non-vacuity control naming every new test.
...
- **D16. The base anchor was moved by the origin/main merge.** After this plan cleared preflight, origin/main advanced 31 commits ahead of the original anchor `04a54e681bd21e841e124c016df30672ee701b75` and was merged into this branch as `bb1c7d4b60f7b782227956f36859314d5c47bb03`. The original anchor is no longer a usable diff anchor: an anchored `git diff --name-only` over the merged tree would enumerate main's own source files as well as this item's, so the P7-T9, P7-T12 and P8-T12 write-set gates would be unsatisfiable against it. `$Base` is therefore the merge commit. The merge changed exactly one write-set file, `UtilitiesCS.Test/UtilitiesCS.Test.csproj`, which gained three `<Compile Include>` items (`TestHelpers\UiThreadStateScope.cs`, `OutlookObjects\Folder\FolderPredictorArchiveRootDegradationTests.cs`, `Threading\UiThreadInitContract_Tests.cs`) from the already-merged items #809 and #812. Every project-file line count in this plan was raised by three (985 in the merged base tree, 986 after P1-T7, 987 after P2-T2, 988 after P6-T4) and the item citations moved to 74-77, 189-192 and 541. Item #798, which this plan's `DfDeedle` citations depend on, merged as `4a29d7e7` BEFORE the original anchor, so it was already present when the plan was authored and its files are unaffected. All sixteen write-set `.cs` line counts and `TimeOutTask.cs` were re-measured against the merged tree and are identical to the values P0-T6 asserts, so P0-T6 is unchanged; it carries no project-file line count. If origin/main advances again mid-execution, the executor stops at the phase boundary and returns to the orchestrator for a fresh reconciliation rather than re-anchoring on its own.
...
- Every `git diff` is anchored to `$Base`. `$Base` is the merge commit that brought origin/main onto this branch, and it is also the value of `HEAD` at the start of execution, because no commit occurs before P7-T12. P3-T3's numstat over spec.md stays written against `HEAD`, which therefore resolves to the same commit as `$Base` when that task runs; spec.md is tracked at both, so the numstat is well-defined against either. Every name-listing diff is paired with `git add --intent-to-add` for the three new test files and a `git status --porcelain --untracked-files=all` span in the same task.
...
- [x] [P8-T1] AC4 runs 1 and 2 into `coverage/trx/p8-t1` (`p8-t1-run1.trx`, `p8-t1-run2.trx`), artifact `<FEATURE>/evidence/regression-testing/p8-t1-ac4-runs.md`. Acceptance: the pair acceptance above; additionally `Read-TrxOutcome` on `p8-t1-run1.trx` is `Passed` for all nine C4 names (the non-vacuity control: an unregistered or undiscovered test would read `ABSENT`, and `ABSENT` fails this clause).

- [x] [P8-T2] AC4 runs 3 and 4 into `coverage/trx/p8-t2`, artifact `<FEATURE>/evidence/regression-testing/p8-t2-ac4-runs.md`. Acceptance: the pair acceptance.

- [x] [P8-T3] AC4 runs 5 and 6 into `coverage/trx/p8-t3`, artifact `<FEATURE>/evidence/regression-testing/p8-t3-ac4-runs.md`. Acceptance: the pair acceptance.
...
- [x] [P8-T5] AC4 runs 9 and 10 into `coverage/trx/p8-t5`, artifact `<FEATURE>/evidence/regression-testing/p8-t5-ac4-runs.md`. Acceptance: the pair acceptance.
...
- No project-file line count enters the 500-line audit; `UtilitiesCS.Test.csproj` is 985 in the merged base tree and grows by exactly three items.
...
- `UtilitiesCS.Test/UtilitiesCS.Test.csproj` | 74-77 (`TestHelpers\` items), 189-192 (`DfDeedle` family), 541 (`OlTableExtensions_Tests.cs`); line count 985
...
CITATION: UtilitiesCS.Test/UtilitiesCS.Test.csproj | TestHelpers items 74-77; DfDeedle items 189-192; OlTableExtensions_Tests item 541; line count 985
