...
- Every `git diff` is anchored to `$Base`. Every name-listing diff is paired with `git add --intent-to-add` for the three new test files and a `git status --porcelain --untracked-files=all` span in the same task.
...
New test methods (fully qualified; the ten names P8-T6 asserts, eight new plus two retained sentinels):
...
- `UtilitiesCS.Test.OutlookObjects.Table.OlTableExtensions_Tests.EnumerateTable_NullWriter_WritesToConsoleAndMovesToStart` (new)
...
- [ ] [P2-T2] Register the new test file in `UtilitiesCS.Test/UtilitiesCS.Test.csproj`: insert `<Compile Include="Extensions\DfDeedleEtlTimeoutTests.cs" />` immediately after the line-190 item `Extensions\DfDeedleQfcColumnTimeoutTests.cs`. Acceptance: `Select-String -SimpleMatch 'Extensions\DfDeedleEtlTimeoutTests.cs'` on the project file counts 1; line count 984.
...
- [ ] [P3-T3] Check off AC2 in `<FEATURE>/spec.md` line 293 (`- [ ] AC2:` becomes `- [x] AC2:`, text unchanged), citing `p1-t9-seam-scoped-run.md` (static seams replaced; `TableEtlInvoker`=0 in production and test files) and `p3-t2-ac2-pass-after.md` (guard). Acceptance: `Select-String -SimpleMatch '- [x] AC2:'` on spec.md counts 1 and `'- [ ] AC2:'` counts 0; no other line of spec.md changed (`git diff --numstat $Base -- docs/features/active/2026-09-07-utilitiescs-test-determinism-780-803-594-811/spec.md` reports 1 insertion and 1 deletion).
...
Every citation below was re-derived against this worktree in this authoring pass with Read, Grep and
Glob; nothing is carried forward from the caller's message, the research artifact, or an earlier
round without an independent read. Sibling regions of each edited citation were re-read (the whole
of `OlTableExtensions.Etl.cs`; `DfDeedle.cs` lines 40-229; `DictionaryExtensions.cs` lines 110-189;
`DfDeedle_COM_Tests.cs` lines 1-60, 193-212, 360-495, 722-854; `OlTableExtensions_Tests.cs` lines
1-30, 930-1019, 1110-1144, 1625-1656, 1700-1855; `DfDeedleQfcColumnTimeoutTests.cs` lines 1-180,
260-330, 470-500; `PrettyPrint.cs` lines 1-40 plus token probes; the four console test files in full
or at their capture sites; `NLogTraceWriter_Test.cs` lines 1-65).

Findings from the pass that changed the plan:
...
CITATION: UtilitiesCS/OutlookObjects/Table/OlTableExtensions.Etl.cs | EtlAsync 66-130; EtlAsyncOld 132-169; EtlByRowAsync 171 and 229-263; TimeoutAfter at 114, 245, 259
...
CITATION: UtilitiesCS.Test/Extensions/DfDeedle_COM_Tests.cs | header 20-33; seam swaps 399-414, 762-782, 827-847; failing test 423-489; line count 869
...
CITATION: UtilitiesCS.Test/OutlookObjects/Table/OlTableExtensions_Tests.cs | attribute 17-20; Returns(120) 960-963; reflection 1124-1137; console capture 1635-1651; builders 1723-1793; line count 1855
CITATION: UtilitiesCS.Test/ReusableTypeClasses/StackGeek_Tests.cs | attribute 9-15; console capture 146-168
CITATION: UtilitiesCS.Test/HelperClasses/PrettyPrint_Tests.cs | attribute 14-19; console capture 185-220
...
The line above is not a discovered defect: executor preflight has not yet run, and the planner does not self-issue clearance. The MCP validator was not on this planner's tool surface (VALIDATOR NOT RUN); a structural self-check against the atomic-plan contract (phase headings, sequential task IDs, canonical evidence paths, one AC per check-off task, anchored diffs, staged companions, before/after observation on the write-mode formatter) was performed instead and is reported separately from the validator signal.
