...
Nine files (created or changed); the largest is approximately 430 lines, well
...
5. `TaskController.Accelerator.cs` — `partial class`: keyboard handlers,
   `SuppressKeystrokes`, `MouseFilter_FormClicked`, `AutoAssignAllAsync`, the
   Keyboard UI region (`ToggleXl`, `UpdateCaptions`, `ExecuteXlAction`,
   `ToggleXlGroupNav`, `DeactivateActiveXlGroup`, `ActivateXlGroup` x3,
   `RecurseXl`), accelerator init, and the `PostMessage` P/Invoke. WinForms-bound;
   uses the `Form` accessor (~430 lines).
6. `TaskController.ControlMaps.cs` — `partial class`: `GetOptionsLookup`/
   ...
   the `ControlRelationship` struct, `OptionsGroups`, `NavTips`. WinForms-bound
   (live `Label`/`TipsController`); uses the `Form` accessor (~400 lines).
...
- The single `private TaskViewer Form => (TaskViewer)_viewer;` accessor is
  confined to the WinForms-bound partials (files 5 and 6) so all WinForms-
  identity access is isolated in exempt code while the testable core sees only
  `ITaskViewer`.
...
- **Files/modules expected to change**: the nine files enumerated in Scope
  (`ITaskViewer.cs`, `TaskController.cs`, `TaskController.Actions.cs`,
  `TaskController.Flags.cs`, `TaskController.Accelerator.cs`,
  `TaskController.ControlMaps.cs`, `TaskDurationParser.cs`,
  `TaskPriorityMapper.cs`, `ITagPromptService.cs` + `TagPromptService.cs`), plus
  `TaskViewer.cs` (add `: ITaskViewer`, facade delegation, relocated accept/cancel
  wiring), and the production and test `.csproj` `<Compile>` groups. No changes to
  `FlagTasks.cs` (constructor compatibility preserved via optional-with-default
  seam parameters).
- **Public interfaces/contracts affected**: `TaskController` constructor
  ...
  public `ITaskViewer` and `ITagPromptService` interfaces.
...
The research proposes file-level `[ExcludeFromCodeCoverage]` on the two
WinForms-bound partials (`TaskController.Accelerator.cs` ~430 lines and
`TaskController.ControlMaps.cs` ~400 lines — approximately 900 lines of
accelerator / control-map code). This spec constrains that proposal as follows:
...
overloads), the assign/shortcut model updates, and the `OK_Action`/`Cancel_Action`
decisions. Removal of the current class-level `[ExcludeFromCodeCoverage]` (source
line 20) is required so these lines are measured.
...
- **Regression tests to add or update**: new MSTest files under
  ...
  `TaskDurationParserTests.cs`, `TaskPriorityMapperTests.cs`), added to the test
  `.csproj` `<Compile>` group. The `Z.Disabled` namespace is not reused. Test
  doubles: `Mock<ITaskViewer>` via Moq with `SetupAllProperties()` and explicit
  `InvokeRequired => false` (no real forms), injected `Action<string>` notifier,
  `Mock<ITagPromptService>`, `Func<MailItem, Task<MailItemHelper>>` factory stub,
  and `Mock<IAutoAssign>`. Reuse the `MoqOlToDo` builders for
  `Categories`/`MailItem`/`ToDoItem`.
...
- **Error handling and logging verification**: the `CaptureDuration` invalid path
  ...
  only through seams.
- **Coverage impact and targets**: the refactored core (files 2–4, 7–8, and the
  ...
  The exempt accelerator/control-map partials are removed from the denominator
  only via the ratified, per-region-justified exemption inventory.
- **Determinism constraints**: no live `Form`/`Control` instantiation; no popups
  (`MessageBox.Show` and `TagViewer.ShowDialog` replaced by seams); no
  `Thread.Sleep`/`Task.Delay` (already banned by BannedApiAnalyzers);
