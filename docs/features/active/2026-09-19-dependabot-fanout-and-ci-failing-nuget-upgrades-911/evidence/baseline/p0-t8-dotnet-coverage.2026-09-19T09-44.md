...
RESOLVED_PATH=<user-home>\.dotnet\tools\dotnet-coverage.exe
...
| Item | Value |
|---|---|
| Resolved command path | `<user-home>\.dotnet\tools\dotnet-coverage.exe` |
...
- `Get-Command dotnet-coverage` resolves to a path:
  `<user-home>\.dotnet\tools\dotnet-coverage.exe`. PASS.
...
it to `<user-home>\.dotnet\tools\dotnet-coverage.exe`, version
