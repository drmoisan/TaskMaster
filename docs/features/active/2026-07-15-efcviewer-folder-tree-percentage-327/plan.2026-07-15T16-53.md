...
- [ ] [P3-T6] Run the full C# toolchain loop in order (`csharpier .` -> analyzers `msbuild` -> nullable `msbuild` -> `vstest.console.exe /EnableCodeCoverage`) and record the green pass in `docs/features/active/2026-07-15-efcviewer-folder-tree-percentage-327/evidence/qa-gates/phase3-toolchain.md`
  - Acceptance: artifact contains `Timestamp:`, `Command:`, `EXIT_CODE:` per step, `Output Summary:`; all four steps green in a single pass.
