...
- **Execution branch:** `feature/quickfiler-breadcrumb-bridge-coverage-r3`
...
- **Sibling boundaries — do not edit.** F13 (#455) owns all `BreadcrumbDropDown*`,
  ...
  `QuickFiler.Test/Viewers/BreadcrumbPopupUiOperationsDirectAdapterTests.cs`**. The
  `BreadcrumbPopup*` glob covers F13's **existing** files only. The new
  `QuickFiler.Test/Viewers/BreadcrumbPopupLifecycleOperationsTests.cs` created by `[P5-T13]`
  is **F12-owned** per AC-11, because its subject type `BreadcrumbPopupLifecycleOperations`
  is declared in the F12-owned `BreadcrumbItemViewerLifecycleCoordinator.cs`; `[P6-T10]`
  records `F13-OWNED PATHS: 0` on that basis. F14 (#456) owns
...
- **D-2. RT-J5, RT-J6, LC-G10(d) and LC-G11(b) are not scheduled.** They appear in the Irreducible
  Outcomes table. This costs `BreadcrumbBridgeRouter.cs` 4 branch outcomes and 1 line (90 - 86 = 4, matching the four router entries in the Irreducible Outcomes table; `spec.md` §4.1's coarser item granularity folds `:372` c1/c2 into one item and so reads 3) (final 99.65%
...
- [ ] [P0-T4] Read `CLAUDE.md`, `.claude/rules/general-code-change.md`, `.claude/rules/general-unit-test.md`, `.claude/rules/csharp.md`, `.claude/rules/quality-tiers.md`, and `.claude/rules/tonality.md` in that order, because `.claude/rules/csharp.md` is path-scoped to `**/*.cs` and `**/*.csproj` and this plan touches both, making it step 4 of the `policy-compliance-order` reading order; acceptance: `<FEATURE>/evidence/baseline/phase0-instructions-read.<ts>.md` records `Timestamp:`, `Policy Order:`, and the explicit list of the six files read
...
- [ ] [P2-T7] Run `dotnet tool run csharpier format QuickFiler.Test/Viewers/BreadcrumbBridgeCoordinatorGuardTests.cs`, then verify the post-format line count and the csproj edit; acceptance: `<FEATURE>/evidence/qa-gates/phase2-file-size-and-csproj.<ts>.md` records `EXIT_CODE: 0`, a line count `<= 500` (projected 200-230), 6 `[TestMethod]`/`[DataTestMethod]` declarations with 8 executions, a csproj entry count of 108, full CRLF, and a diff hunk confined to the single inserted line adjacent to the `Viewers\BreadcrumbBridgeCoordinatorProbabilityTests.cs` anchor (AC-7, AC-15)
- [ ] [P2-T8] Run the scoped test pass with `& <vstest-path> QuickFiler.Test\bin\Debug\QuickFiler.Test.dll /Settings:scripts\vscode\TaskMaster.cli.runsettings /InIsolation /TestCaseFilter:"FullyQualifiedName~BreadcrumbBridgeCoordinatorGuardTests|FullyQualifiedName~BreadcrumbBridgeCoordinatorTests|FullyQualifiedName~BreadcrumbBridgeCoordinatorProbabilityTests"`; acceptance: `<FEATURE>/evidence/regression-testing/phase2-scoped-run.<ts>.md` records `Timestamp:`, `Command:`, `EXIT_CODE: 0`, 8 new executions passed, and zero failures across the pre-existing coordinator tests
...
- [ ] [P5-T4] Add `[TestMethod] ConfigureHost_NullArguments_ThrowBeforeAnythingIsPosted` (LC-G2c) to `QuickFiler.Test/Viewers/BreadcrumbItemViewerLifecycleCoordinatorGuardTests.cs` asserting `ArgumentNullException` `.WithParameterName("host" | "anchorBounds" | "workingArea")` **and** that the pumped context's queue is still empty after each throwing call — read via the `PendingCount` member of the F12-local context declared in `[P5-T1]` — so a guard placed after the post would fail; acceptance: the test passes and closes `BreadcrumbItemViewerLifecycleCoordinator.cs:115`, `:116`, `:117` (AC-1, AC-2, US-3)
- [ ] [P5-T5] Add `[TestMethod] Focus_NullAction_ThrowsBeforeAnythingIsPosted` (LC-G2d) to `QuickFiler.Test/Viewers/BreadcrumbItemViewerLifecycleCoordinatorGuardTests.cs` asserting `ArgumentNullException` `.WithParameterName("focus")` and an empty context queue afterwards via the same `PendingCount` member; acceptance: the test passes and closes `BreadcrumbItemViewerLifecycleCoordinator.cs:165` (AC-1, AC-2, US-3)
...
- [ ] [P6-T4] Verify AC-6 and AC-7 determinism by scanning every test file added or modified by this child for `Thread.Sleep`, `Task.Delay`, wall-clock waits, real-time polling, temporary files, filesystem writes, external services or processes, `.Show()`, `.ShowDialog()`, popups, STA attributes, `*.StaTests.cs` filenames, `GC.Collect`, injected clocks, `TimeProvider`, and `FakeTimeProvider`, and confirming every ambient `SynchronizationContext` assignment is restored in a `finally`; acceptance: `<FEATURE>/evidence/qa-gates/ac06-ac07-determinism.<ts>.md` records a zero-match result for every prohibited pattern across the **seven** touched test files — the five new files, the Phase 1 extension `QuickFiler.Test/Viewers/BreadcrumbCoordinatorUpgradeLifetimeTests.cs`, and the rename-only `QuickFiler.Test/Viewers/BreadcrumbItemViewerLifecycleCoordinatorTests.cs` from `[P5-T29]`, matching the seven files `[P7-T10]` enumerates — and enumerates each ambient-context site with its `finally` restore (AC-6, AC-7, US-3)
...
- [ ] [P6-T9] Verify AC-14 retain-or-improve fixture guards not already covered by `[P3-T9]`, namely that no F12 change weakened, disabled, or deleted any pre-existing assertion anywhere in `QuickFiler.Test`; acceptance: `<FEATURE>/evidence/qa-gates/ac14-no-weakened-assertions.<ts>.md` records a zero count of removed or weakened pre-existing assertions across the diff, confirms that the only modification to a pre-existing test **method** is the single-line rename in `[P5-T29]`, and confirms that the only other change to a pre-existing test file is the purely additive Phase 1 extension of `QuickFiler.Test/Viewers/BreadcrumbCoordinatorUpgradeLifetimeTests.cs` (`[P1-T1]` .. `[P1-T5]`), which adds one private helper class and four `[TestMethod]` declarations and alters no pre-existing statement (AC-14, US-7)
...
- **Unit (new, MSTest + Moq + FluentAssertions, Arrange–Act–Assert with explicit section comments):**
  ...
  - `QuickFiler.Test/Viewers/BreadcrumbBridgeCoordinatorGuardTests.cs` — **new**, 6 declarations
    (BC-G1 .. BC-G6), 8 executions counting `[DataRow]`s.
  ...
  - **Totals: 46 new declarations, 54 executions, 5 new files, 1 extended file, 5 new csproj entries.** Per-phase executions: 4 (UL) + 8 (BC) + 5 (HUB) + 6 (RT) + 16 (LC guard) + 15 (LC ops) = 54.
