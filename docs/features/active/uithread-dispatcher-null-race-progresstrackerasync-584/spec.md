...
- Customer impact and severity: no end-user impact has been assessed; the observed failure is in the
  ...
  (`UtilitiesCS/Threading/UiThread.cs`, `UtilitiesCS/Threading/ProgressTrackerAsync.cs`), not test
  ...
  Test-suite impact: a non-deterministic failure erodes trust in the full-suite gate (`.claude/rules/general-unit-test.md`
  Core Principles 3 Fast Execution / 4 Determinism), because it cannot be distinguished from a real
...
- Logs/screenshots/error snippets: `NullReferenceException` at
  `UtilitiesCS/Threading/ProgressTrackerAsync.cs:35` on an STA thread, 793 ms into the failing run
...
- Explicitly excluded systems, integrations, or datasets: `.claude/**`, `.codex/**`, `.agents/**`,
  `config/blast-radius.json`, `config/orchestration-routing.json` (published from upstream
...
- Signals/evidence supporting it: verified directly against `origin/main` at
  ...
  `UtilitiesCS/Threading/UiThread.cs:1-163` and `UtilitiesCS/Threading/ProgressTrackerAsync.cs:1-104`
  in full. The existing repository idiom for the same hazard already exists at
  `UtilitiesCS/OutlookObjects/Folder/WpfDispatcherYield.cs:57-66`, which explicitly comments: "
...
- Affected components/modules: `UtilitiesCS/Threading/UiThread.cs` (the accessor); consumers reading
  ...
  `UtilitiesCS/Threading/IdleAsyncQueue.cs`, `UtilitiesCS/HelperClasses/ThemeHelpers/*`,
  `UtilitiesCS/HelperClasses/ToolTips/QfcTipsDetails.cs`, several `QuickFiler/Controllers/*` files,
  and `TaskMaster/Ribbon/RibbonViewer.EngineCommands.cs`, `TaskMaster/AppGlobals/*`,
...
- The new failure is a named `InvalidOperationException` with a message stating the missing
  ...
  `WpfDispatcherYield.cs:64-66`. No new logging is added; existing catch sites (for example
...
- Constraints (budget, performance, compatibility): 1-3 production files, per the assignment's
  small-path budget; no `.claude/**`/`.codex/**`/`.agents/**`/`config/blast-radius.json`/
  `config/orchestration-routing.json` edits.
