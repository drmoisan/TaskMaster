...
- **Branch:** `feature/quickfiler-per-file-coverage-capstone-r3`
...
- **Worktree:** `C:\Users\DanMoisan\repos\TaskMaster\.claude\worktrees\agent-a8fd73e5a8544c318`
- **Plan file (updated in place):** `<FEATURE>/plan.2026-08-08T00-34.md`
- **Upstream dependencies:** all fifteen sibling children (F1-F15) merged to the integration branch; F1's ledger and harness — consumed at **execution time**, never at planning or preflight time
...
| --- | --- | --- | --- |
| 0 | Compliance, Restore, Upstream Dependency Confirmation, and Baseline Capture | 22 | baseline contract |
...
| 3 | Capstone-Owned Repository-Wide Recomputation Tooling | 12 | spec § Definition of Done |
...
- **Every negative or absence claim carries `SearchScope:`, `SearchPatterns:`, `SearchResult:`.**
  Most of this plan's cross-sibling audits consist of asserting something is absent across fifteen
...
| D-6 | **The epic manifest's placeholder issue numbers are REPORTED, not repaired.** Re-verified 2026-08-08 against the manifest on this branch: F12 (`495`) and F15 (`496`) have since been back-filled and F16's own `depends_on` list now names real issue numbers throughout. The sole remaining placeholder is F16's own entry — `issue_num: 1016` against the real **#497**, and `feature_folder: quickfiler-per-file-coverage-capstone` against the real `2026-08-08-quickfiler-per-file-coverage-capstone-497`. `epic.md` is not a per-child owned file; F16 verifies the repair and never performs it. | `spec.md` DEV-6; research 2 § Q7.1; epic.md:55-83 |
...
- [ ] [P0-T1] Read `CLAUDE.md` in full and create `<FEATURE>/evidence/baseline/phase0-instructions-read.<TS>.md` containing `Timestamp:`, `Policy Order:` (the `policy-compliance-order` sequence), and an explicit list of files read with `CLAUDE.md` as the first entry.
  - Acceptance: the artifact exists at that exact path, contains all three fields, and records the §UT2 coverage exemption grounds (a), (b), (c), the §UT2 `>= 90%` new-module coverage target, and the §CUT3 toolchain command list as the clauses that bind this child.
...
- [ ] [P0-T14] Confirm the fifteen sibling children (F1-F15) have merged by recording, into `<FEATURE>/evidence/baseline/sibling-fan-in.<TS>.md`, the resolved feature-folder path for each `features[]` entry in `<EPIC>/epic.md` other than F16.
  - Acceptance: the artifact lists every `features[]` entry other than F16 — fifteen at planning time — with the count re-derived from the manifest at execution time rather than asserted, the directory each resolves to under `docs/features/active/` or `docs/features/completed/`, and `SearchScope:` / `SearchPatterns:` / `SearchResult:` for every one that does not resolve. F12 (495) and F15 (496) manifest folder names now match their on-disk folders; F2's entry (`quickfiler-queue-admission-coverage`) carries no issue-number suffix, so resolution is by issue-number suffix first and slug match second, and the resolution rule used is written into the artifact. An unresolved child is recorded as a `BLOCKING` epic-sequencing finding, not repaired.
...
- [ ] [P0-T22] Capture the PowerShell baseline before any capstone-owned script exists by running `mcp__drm-copilot__run_poshqc_analyze` and then `mcp__drm-copilot__run_poshqc_test` over the repository and recording `<FEATURE>/evidence/baseline/poshqc-baseline.<TS>.md`.
  - Acceptance: all four schema fields, with `EXIT_CODE:` carrying the tool-reported status because the MCP surface returns no process exit code, and `Output Summary:` recording the analyzer diagnostic count and the numeric passed and failed Pester counts. This is the baseline [P13-T9] and [P13-T10] are evaluated against; a diagnostic or failure carried unchanged from this baseline is `INFORMATIONAL` and is not this child's to fix.
...
- [ ] [P7-T11] Apply the nine rejection tests to every `ratified-exempt` row and record the per-row result in the same artifact.
  - Tests: (1) a seam exists or is feasible; (2) the ground does not textually cover the file; (3) the row reaches a `CLAUDE.md`:303 named non-exempt seam; (4) a ground-4 conjunct fails; (5) a `ratified-exempt` row relies on a member-level attribute (per the [P6-T3] classification) rather than a type-level one; every such row is recorded `PENDING-P9` at this phase and its pass/fail is resolved by the #457 lambda analysis in Phase 9; (6) `ratified-exempt` is asserted on a zero-coverable-lines file; (7) the exemption closes a coverage gap discovered during execution; (8) a `coverage.config` or `.runsettings` assembly-level exclude touches QuickFiler; (9) a newly created file claims exemption without a ground meeting the standard.
  - Acceptance: each test is applied to each row with a recorded pass/fail, except test (5) on member-level rows, which is recorded `PENDING-P9`; each failure is `BLOCKING` naming the owning child; and the artifact states verbatim that **none of the nine may be waived by F16 and F16 may never itself grant an exemption to close a gap**.
...
- [ ] [P7-T13] Write the phase verdict into `<FEATURE>/evidence/qa-gates/ac3-exemption-ground-reconciliation.<TS>.md`, closing `spec.md` AC3.
  - Acceptance: the verdict records the `ratified-exempt` row count, the per-ground row counts, the U1-U4 pass counts, the nine rejection-test failure counts, and the total Blocking count with each owning child named. A non-zero Blocking count makes the phase verdict `BLOCKED`. The rejection-test failure count for test (5) is provisional until [P9-T5] back-fills it, and the verdict records that dependency explicitly.
...
- [ ] [P9-T5] Write the phase verdict into `<FEATURE>/evidence/qa-gates/ac457-lambda-residual-scan.<TS>.md`.
  - Acceptance: the verdict records the stage-1 candidate count, the stage-2 confirmed count, the count of files whose cap falls below 0.80, and each owning child. The artifact states verbatim that **the disposition — a class-level-exempt, non-`partial` adapter type — belongs to the owning child and F16 performs no refactor**, and cites open issue **#457**. Back-fill the resolved test-(5) verdict for every row [P7-T11] recorded `PENDING-P9` into `<FEATURE>/evidence/qa-gates/ac3-exemption-ground-reconciliation.<TS>.md`, and update the [P7-T13] rejection-test failure count and Blocking count accordingly.
...
- [ ] [P10-T1] Carry the sibling-folder resolution map from [P0-T14] into `<FEATURE>/evidence/qa-gates/ac2-per-file-artifact-audit.<TS>.md` and record, per child, the resolved folder path, the `research/` directory path, and the `plan.*.md` path.
  - Acceptance: all fifteen children are listed; any child with no resolved folder, no `research/` directory, or no plan file carries `SearchScope:` / `SearchPatterns:` / `SearchResult:` and is a `BLOCKING` epic-sequencing finding naming that child.
...
- [ ] [P12-T8] Verify the epic manifest's placeholder issue numbers are resolved and append the result to the same artifact.
  - Acceptance: `<EPIC>/epic.md` YAML front matter is read at execution time; the `issue_num` for F12, F15, and F16 is checked against **495**, **496**, and **497** respectively; F16's `depends_on` list is checked to contain 495 and 496 and to carry no `1012`/`1015` placeholder; and F16's own `feature_folder` is checked against `2026-08-08-quickfiler-per-file-coverage-capstone-497`. Every unresolved placeholder is a `FINDING` naming the epic manifest owner, recorded with the reason it matters: an epic-orchestrator dependency gate keyed on `depends_on` will fail to find a nonexistent issue and will either error or silently skip the dependency. **`epic.md` is not edited by this child** (D-6), and `git status --porcelain` is re-run to prove it is unmodified.
...
- [ ] [P13-T10] Run `mcp__drm-copilot__run_poshqc_test` and record `<FEATURE>/evidence/qa-gates/toolchain-ps-test.<TS>.md`.
  - Acceptance: the artifact records the passed and failed counts evaluated baseline-relative against [P0-T22]: the failed count must not exceed the baseline failed count, any failure carried unchanged from the baseline is `INFORMATIONAL`, and any new failure is `BLOCKING` naming the file; `EXIT_CODE:` records the tool-reported status because the MCP surface returns no process exit code. It also confirms the two new suites appear as `<testsuite>` entries in the emitted Pester JUnit output. The scoped Pester coverage command from the Resolved Toolchain Commands table is additionally run in this final pass with `$c.CodeCoverage.OutputPath = "docs/features/active/2026-08-08-quickfiler-per-file-coverage-capstone-497/evidence/qa-gates/toolchain-ps-test.<TS>.xml"`, and the numeric post-change line-coverage percentage for the two capstone-owned production scripts is recorded and must be `>= 90.0`; the [P3-T12] figure is the pre-final-pass reference, not the final-QC value.
...
- **Coverage evidence:**
  ...
  - Capstone-owned PowerShell: `<FEATURE>/evidence/baseline/poshqc-baseline.<TS>.md` (baseline,
    [P0-T22]), `<FEATURE>/evidence/qa-gates/capstone-tooling-coverage.<TS>.md` and
    `capstone-tooling-coverage.<TS>.xml` ([P3-T12], pre-final-pass reference), and
    `<FEATURE>/evidence/qa-gates/toolchain-ps-test.<TS>.md` and `toolchain-ps-test.<TS>.xml`
    ([P13-T10], the final-QC value).
...
3. **The research-recorded sibling-folder absences are stale.** Re-verified 2026-08-08 on this
   branch: all fifteen non-F16 `feature_folder` values in the manifest resolve exactly under
   `docs/features/active/`, F15 (`2026-08-08-quickfiler-form-viewers-bayesian-coverage-496`)
   included. [P0-T14] re-derives the map at execution time rather than relying on any
   planning-time list.
4. **One manifest placeholder remains, not three.** Re-verified 2026-08-08: F12 (`495`) and F15
   (`496`) have been back-filled and every `depends_on` edge now names a real issue number. The
   only unresolved entry is F16's own — `issue_num: 1016` against the real **#497**, with
   `feature_folder: quickfiler-per-file-coverage-capstone` against the real
   `2026-08-08-quickfiler-per-file-coverage-capstone-497`. Per D-6 and Hard Prohibition 6 this is
   REPORTED, never repaired by F16. [P12-T8] re-reads the manifest rather than trusting this count.
