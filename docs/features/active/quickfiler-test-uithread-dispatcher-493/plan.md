...
4. **No task asserts a source-text signature literal that the same task's instruction changes.**
   ...
   `typeof(UiThread)`, `BindingFlags`, `System.Reflection`, `[TestMethod]`,
...
- **D5 — coverage is reported, not floored, and the denominator is checked before the rate is.**
  ...
  That gate is conditional on the denominator being comparable, because the tool is known to be
...
- [ ] [P1-T1] Create `QuickFiler.Test/Controllers/QfcItemController.UiThreadDispatcherFixture.cs` in namespace `QuickFiler.Controllers.Tests`, containing `internal static class UiThreadDispatcherFixture` and `internal sealed class UiThreadDispatcherTransaction : IDisposable`, implemented exactly to § Fixture Contract below. Every field declaration listed there carries its initializer at the declaration or is assigned in the single constructor named there; no field may be declared without one. Move `_dedicatedDispatcher`, `_dedicatedDispatcherLock`, and `GetDedicatedDispatcher` out of `QfcItemController.TestSupport.cs` into this file, renamed `_parkedDispatcher`, `ParkedDispatcherLock`, and `GetParkedDispatcher`; leave `StartRunningDispatcher` and `ShutdownDispatcher` in `QfcItemControllerTestSupport`, because the unowned `WpfUiDispatcherTests.cs` calls them. **Acceptance:** the file exists at that path and `Select-String -SimpleMatch -Pattern 'typeof(UiThread)' -Path 'QuickFiler.Test/Controllers/QfcItemController.UiThreadDispatcherFixture.cs'` returns at least one match; the artifact `<FEATURE>/evidence/other/fixture-created.<TS>.md` records that match count and the file's line count.
...
- [ ] [P3-T1] Apply formatting to this plan's own C# paths only: `dotnet tool run csharpier format QuickFiler.Test/Controllers/QfcItemController.UiThreadDispatcherFixture.cs QuickFiler.Test/Controllers/QfcItemController.UiThreadDispatcherFixtureTests.cs QuickFiler.Test/Controllers/QfcItemController.TestSupport.cs QuickFiler.Test/Controllers/QfcItemController.InitializationTests.Part2.cs`. Write `<FEATURE>/evidence/qa-gates/csharpier-format.<TS>.md`. **Acceptance:** the artifact records `EXIT_CODE: 0` and records, for each of the four paths, its SHA-256 before and after this command, so a rewrite is visible as a changed pair rather than inferred from the tool's processed-file count.

- [ ] [P3-T2] Verify formatting repository-wide, read-only: `dotnet tool run csharpier check .` from `WS`. Write `<FEATURE>/evidence/qa-gates/csharpier-check.<TS>.md`. **Acceptance:** the artifact records `EXIT_CODE: 0` together with the verbatim final summary line.

- [ ] [P3-T3] Run the analyzer gate: `& $MSBUILD TaskMaster.sln /t:Rebuild /m /p:Configuration=Debug "/p:Platform=Any CPU" /p:EnableNETAnalyzers=true /p:EnforceCodeStyleInBuild=true` with output redirected to `TestResults/plan-logs/p3-t3/msbuild-analyzers.log`. Write `<FEATURE>/evidence/qa-gates/msbuild-analyzers.<TS>.md`. **Acceptance:** the artifact records `EXIT_CODE: 0`, records the total warning count and total error count from the msbuild summary, and names the log path `TestResults/plan-logs/p3-t3/msbuild-analyzers.log` for consumption by `P4-T2`.
...
- [ ] [P5-T9] Verify **AC-9**, the full C# toolchain passing in a single final pass in order, against the six Phase 3 artifacts, then check AC-9 off in `<FEATURE>/spec.md` and record in the completion report the four commands run, verbatim, in the order csharpier check, analyzer msbuild, `TreatWarningsAsErrors` msbuild, vstest with coverage. **Acceptance:** all six Phase 3 artifacts exist, each records `EXIT_CODE: 0` with `P3-T5` additionally satisfying its subset comparison, no Phase 3 artifact records `SKIPPED`, and exactly one further checkbox in `spec.md` changed state.
...
- `internal static Dispatcher Current { get; }` — reads `DispatcherField` inside `lock (FieldLock)`.
...
- `using System.Windows.Threading;` (line 7) is deleted for the same reason. Every bare `Dispatcher`
  type name in this file today is removed by the edits above — the local at line 138, the
  `SwapUiThreadDispatcher` signature and body at lines 148-155, and the `PumpHarness` field,
  constructor parameter, and assignment at lines 308, 316, and 323. What remains is `IUiDispatcher`
  at lines 365, 390, and 413, which comes from `UtilitiesCS.Threading`, not from
  `System.Windows.Threading`. Leaving the directive would contradict the reason this task deletes
  `using System.Reflection;`.
- Every other using directive is left unchanged.
