...
- [ ] [P8-T2] Apply nullable annotations, guards, justified `!`, and the ratified `where TKey : notnull` constraint (from [P6-T2]) to the TWO truly generic Batch-8 dictionary bases `ScoDictionaryNew<TKey, TValue>` (already constrained in the committed batch) and `ScDictionary<TKey, TValue>` (the remaining base) so each of the 7 Batch 8 files reaches zero CS86xx and zero CS8714 under the pragma. FACTUAL CORRECTION: `ScoDictionaryStatic` is a NON-GENERIC `static class` of Type extension methods with no `TKey` type parameter, so the `where TKey : notnull` constraint is mechanically INAPPLICABLE there and MUST NOT be added; the [P6-T2] "four generic bases" wording is inaccurate for this file, and the constraint in fact applies to the three truly generic bases (`ConcurrentObservableDictionary` [Batch 6], `ScoDictionaryNew`, `ScDictionary`). `ScBag` (`ConcurrentBag<T>`-based) takes `T` and MUST NOT receive the `notnull` constraint. Annotate serialization round-trip fields (`Name`, `ism`) as `= null!` with a `// set by deserialization` comment, uninitialized events as `?`, `null`-literal `altLoader` params as `Func<...>? altLoader`, and `SloLinkedList` `NotImplementedException` interface stubs' parameters to match the interface nullable contract without implementing bodies. EPIC-AUTHORIZED THREE-FILE CROSS-CHILD WAIVER: constraining `ScDictionary` cascades CS8714 into a THIRD #367-owned NewtonsoftHelpers consumer, `UtilitiesCS/NewtonsoftHelpers/WrapperScDictionary.cs`; under explicit epic-layer authorization (Option A-prime) add exactly one `where TKey : notnull` line to `public class WrapperScDictionary<TDerived, TKey, TValue>` (declaration at line 18) and nothing else. This is the THIRD file in the cross-child waiver, joining `WrapperScoDictionary.cs` and `ScoDictionaryConverter.cs` (both already constrained); no other cross-child file may be modified, and if a FOURTH cross-child consumer surfaces, STOP and re-escalate to the epic layer. Do NOT touch any other `NewtonsoftHelpers` file
  - Acceptance: no post-condition attribute is added; the `where TKey : notnull` constraint applied consistently to the THREE truly generic bases (`ConcurrentObservableDictionary`, `ScoDictionaryNew`, `ScDictionary`) and NOT to `ScoDictionaryStatic` (a non-generic `static class`; the constraint is inapplicable) or `ScBag`; no `record`/`init` conversion; exactly the three epic-authorized waiver consumers (`WrapperScoDictionary.cs`, `ScoDictionaryConverter.cs`, `WrapperScDictionary.cs`) each receive one additive `where TKey : notnull` line and no other `NewtonsoftHelpers` file is edited; `SerializableList.cs` not split; public signatures behavior-compatible except the additive ratified constraint (AC5); annotation/null-safety only (AC3).
...
- [ ] [P9-T9] Verify the CS8714 constraint consistency and AC6 exemption: confirm the ratified `where TKey : notnull` constraint is present on the THREE truly generic dictionary bases (`ConcurrentObservableDictionary`, `ScoDictionaryNew`, `ScDictionary`) and on the THREE epic-authorized waiver consumers (`WrapperScoDictionary<TDerived, TKey, TValue>`, `ScoDictionaryConverter<TDerived, TKey, TValue>`, and now `WrapperScDictionary<TDerived, TKey, TValue>` in `UtilitiesCS/NewtonsoftHelpers/WrapperScDictionary.cs`), and NOT on `ScoDictionaryStatic` (a non-generic `static class`; the constraint is inapplicable), `ConcurrentObservableBag`, or `ScBag`; confirm no NewtonsoftHelpers file other than those three waiver consumers (`WrapperScoDictionary.cs`, `ScoDictionaryConverter.cs`, `WrapperScDictionary.cs`) was modified; and confirm the three exempt WinForms files (`ConfigViewer.Designer.cs`, `ConfigViewer.cs`, `ConfigGroupBox.cs`) carry no `#nullable enable`; record the result at `docs/features/active/utilitiescs-nullable-reusabletypes/evidence/qa-gates/final-constraint-and-exemption-check.md`
  - Acceptance: artifact contains `Timestamp:`, the grep commands used, per-type confirmation of the constraint placement, confirmation the three exempt files remain null-oblivious (AC5/AC6), and confirmation that the `where TKey : notnull` waiver line is present on `WrapperScoDictionary<TDerived, TKey, TValue>`, `ScoDictionaryConverter<TDerived, TKey, TValue>` (in `UtilitiesCS/NewtonsoftHelpers/ScoDictionaryConverter.cs`), and `WrapperScDictionary<TDerived, TKey, TValue>` (in `UtilitiesCS/NewtonsoftHelpers/WrapperScDictionary.cs`), and that no NewtonsoftHelpers file other than those three was modified.
- [ ] [P9-T10] Verify AC5 signature compatibility by reviewing the git diff of the 51 remediated files and confirming only nullability annotations, the ratified `where TKey : notnull` constraint, and justified `!` changed with no public-signature behavior change, and record the result at `docs/features/active/utilitiescs-nullable-reusabletypes/evidence/qa-gates/final-signature-compat.md`. The THREE one-line additive `where TKey : notnull` constraints on `WrapperScoDictionary<TDerived, TKey, TValue>`, `ScoDictionaryConverter<TDerived, TKey, TValue>`, and `WrapperScDictionary<TDerived, TKey, TValue>` (in `UtilitiesCS/NewtonsoftHelpers/WrapperScDictionary.cs`) are epic-authorized Option-A-prime three-file waiver changes (additive constraint, no behavior change) and are expected in the diff.
