...
| AC-5 | A chord that does not carry the `Keys.Alt` flag is not claimed. | `ClaimsAltChord_WithoutAltFlag_ReturnsFalse` passes with input `Keys.M`. |
| AC-6 | A null handler is not claimed. | `ClaimsAltChord_WithNullHandler_ReturnsFalse` passes with inputs `null` and `Keys.Alt`. |
