...
3. **Nullable / type-check**
   ...
   **`/p:Nullable=enable` must not be added.** No project in this repository carries a `<Nullable>`
   ...
   conscripts every file that has never adopted the `#nullable enable` pragma. The command above
   carries the same target and the same properties as the command CI runs. It is not
   character-for-character identical to it, and CI does not run it from ci.yml: that file only calls
   reusable workflows, and the msbuild invocation lives in .github/workflows/_build-nullable.yml on
   lines 57 through 59, as a backtick-continued pwsh block that resolves the solution through
   `$env:SOLUTION_PATH` rather than naming it literally. Omitting the property loses no
...
scripts/vscode/Invoke-MSTestWithCoverage.ps1 does not carry this defect. Its discovery pipeline at
lines 296-302 wraps the whole expression in `@( ... )`, so `$testAssemblies` is array-valued at any
match count, including exactly one, and the `.Count` reads at lines 304 and 315 cannot throw. The
defect is specific to scripts/vscode/Invoke-MSTest.ps1, which has no such wrap.
...
| AC-10 | The full C# toolchain passes in order: format, analyzers, nullable/type-check, tests. | `dotnet tool run csharpier check .` is read-only, so its exit code 0 is the gate. Both msbuild invocations exit 0, and each build log contains at least one occurrence of the literal `Task "Csc"`, which proves `CoreCompile` actually ran rather than being skipped by incrementality. Each console output carries a summary line matching `^\s*0 Error\(s\)$` and no line matching the MSBuild diagnostic form `: error [A-Z]+[0-9]+:`. For the analyzer invocation, the set of `(source file name, diagnostic identifier)` pairs taken from lines matching `: warning [A-Z]+[0-9]+:` that name one of the three changed files equals the corresponding baseline set captured in Phase 0 under `docs/features/active/qfc-twin-processcmdkey-alt-chord-over-claim-663/evidence/baseline/`. That comparison is baseline-relative rather than an absolute zero because .github/workflows/_build-analyzers.yml runs the analyzer command with no `/p:TreatWarningsAsErrors=true`, so an analyzer warning naming one of those files can already exist on origin/main and an absolute-zero clause would be unsatisfiable without an edit the non-goals above forbid. No warning assertion is made over the type-check invocation: `/p:TreatWarningsAsErrors=true` promotes every compiler and analyzer warning to an error there, so an exit-0 run has by construction emitted none and a warning clause would return the same value whatever was written. A bare search for the word `error` is not used: a successful MSBuild run prints the `/errorreport:prompt` token on every Csc command line and prints its own `0 Error(s)` summary, so that search matches on a clean run and the gate could never pass. `/p:Nullable=enable` is absent from the type-check command. Every wrapper invocation uses `-SearchRoot .`; the single-assembly form is unusable for the reason recorded under Known tooling defect below, so no acceptance depends on it. The repository-wide `Invoke-MSTestWithCoverage.ps1` run's exit code is recorded but is NOT the gate: that script throws when the inner run reports any failure, and the repository carries pre-existing load-driven failures that appear only under the concurrent instrumented run. Its gate is instead that every failing test it reports is a member of the baseline failure set captured in Phase 0 under `docs/features/active/qfc-twin-processcmdkey-alt-chord-over-claim-663/evidence/baseline/`, with no failure outside that set. Console logs recorded under `docs/features/active/qfc-twin-processcmdkey-alt-chord-over-claim-663/evidence/qa-gates/`. |
| AC-11 | Coverage shows no regression on changed lines, and `ClaimsAltChord` meets the `>= 90%` new-method floor. | The Cobertura report produced by Invoke-MSTestWithCoverage.ps1 is written to `docs/features/active/qfc-twin-processcmdkey-alt-chord-over-claim-663/evidence/qa-gates/coverage.cobertura.xml`, post-processed in place, read there, and then deleted: raw Cobertura is machine-generated measurement data of order ten megabytes and is not committed in this repository. The evidence of record is therefore `docs/features/active/qfc-twin-processcmdkey-alt-chord-over-claim-663/evidence/qa-gates/coverage-final.md`, which transcribes verbatim the `<method>` element whose `name` is `ClaimsAltChord` and the `<class>` element whose `filename` ends with `QfcFormKeyHandler.cs` that contains it, and which carries an `AC-11 evidence of record:` line stating that the raw document was transcribed into it and then deleted. This criterion holds when the transcribed `<method>` element's `line-rate` is at least `0.90` and the transcribed class element's `line-rate` is not lower than the pre-change baseline recorded at `docs/features/active/qfc-twin-processcmdkey-alt-chord-over-claim-663/evidence/baseline/coverage.md`, both figures read from the same document kind. |
...
| AC-13 | No new `[ExcludeFromCodeCoverage]` attribute is introduced anywhere in the change. | `git diff -U0 origin/main...HEAD -- '*.cs'` contains no added line (`+` prefix) matching `ExcludeFromCodeCoverage`. The diff is scoped to `.cs` paths because the documentation commits already on this branch add twenty lines that quote the attribute name in prose, so the unscoped form reports twenty `+` matches before any source edit is made and could never pass. Scoping preserves the gate's discrimination, because this criterion is about the change set's C# content. |
...
- **Follow-up candidates (open as separate issues; explicitly not part of this fix).**
  ...
  3. Adding the missing `Keys.Menu | Keys.Alt` positive case to the Email Filer suite at
     ...
     `Keys.Alt` shape.
  4. Already opened as issue #713: the single-assembly search root throws under StrictMode in
     scripts/vscode/Invoke-MSTest.ps1. See Known tooling defect above.
