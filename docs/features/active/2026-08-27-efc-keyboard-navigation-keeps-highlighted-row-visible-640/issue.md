...
- [ ] When a Down-arrow navigation move changes the highlighted Efc row from visible to below the
      scroll viewport, the list scrolls so that the new row is completely visible and as high as
      the available scroll range permits.
- [ ] When an Up-arrow navigation move changes the highlighted Efc row from visible to above the
      scroll viewport, the list scrolls so that the new row is completely visible and as low as
      the available scroll range permits.
- [ ] When an Up- or Down-arrow navigation move leaves the highlighted row completely visible, the
      list does not change its scroll position.
- [ ] The regression coverage verifies the Efc bridge-document contract without changing the
      separate Qfc `FolderBreadcrumb.html` path or the Left/Right behavior tracked by issue #440.
