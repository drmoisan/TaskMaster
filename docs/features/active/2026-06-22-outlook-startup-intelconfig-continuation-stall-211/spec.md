...
- [x] AC10 (Phase 4, evidence-gated): apply the minimal TaskMaster-side fix indicated by AC9, with a unit test asserting the behavior/ordering invariant the fix relies on and a re-capture confirming the startup-latency reduction. — AUTOMATED PORTION VERIFIED (2026-06-24, plan `plan.2026-06-24T17-30.md`). The prior "fix target RETRACTED / SpamBayes withdrawn" note is SUPERSEDED by the confirmed root cause: `LoadJunkCertain` / `LoadJunkPotential` built `new FolderTree(Root)`, recursively enumerating the entire default-store folder hierarchy on the STA before searching (proven JunkCertain = ~50,172 ms cold vs direct `GetDefaultFolder` = 4.4 ms). Fix: a new coverable `JunkFolderPathNavigator` over an `IFolderNode` abstraction performs direct path navigation (BFS-from-root for the first segment, direct-child match for subsequent segments, ordinal case-sensitive `Name ==`), reproducing the legacy `FindSequentialNode` matching semantics exactly; both `LoadJunk*` sites call it via an `[ExcludeFromCodeCoverage]` COM adapter; the not-found fallback (MyBox, PickFolder, WriteJunk*Setting, Save) is preserved verbatim and `FolderTree.cs` is unchanged. Bugfix workflow honored: red-before-green regression (enumeration count 785 -> <=4 on a breadth=5/depth=4 tree; evidence `evidence/regression-testing/{red,green}-run-enumeration-bound-2026-06-24T17-30.md`) plus 5 correctness tests + 4 edge tests. Full C# toolchain passes in order (CSharpier -> analyzers -> nullable/TWAE -> MSTest+coverage `TestCategory!=LiveOutlook`); 4109/4109 tests pass; new-code coverage 94.92% (>= 90%); no repo-wide regression (61.84% -> 61.90%); all touched files <= 500 lines. The runtime re-capture portion is maintainer-gated (`evidence/other/runtime-capture-ac10-junk-navigation-PLACEHOLDER.md` + instructions). `PreserveReferencesHandling.All` is retained regardless.
