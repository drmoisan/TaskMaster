...
- [x] AC9 (runtime, maintainer): a non-debugger cold-start capture produces the per-engine attribution lines and identifies which engine(s)/resource(s) dominate the `Engines`-phase wall-clock; recorded under `evidence/`. — MET. The cold-start capture `evidence/other/runtime-capture-engines-coldstart-2026-06-23T17-42.md` reproduced the multi-minute latency and attributed it to a single engine: **`SpamBayes.CreateEngineAsync` deserialization at ~67.5 s** (Engines phase 1:07.63, TOTAL 1:08.80; Triage 43.7 ms, Actionable 9.5 ms). Warm vs cold for Spam is ~1.3 s vs ~67.5 s, indicating cold-disk model read + `PreserveReferencesHandling.All` deserialization. The earlier warm capture (T17-33) validated the probe but did not reproduce the stall.
- [ ] AC10 (Phase 4, evidence-gated): apply the minimal TaskMaster-side fix indicated by AC9, with a unit test asserting the behavior/ordering invariant the fix relies on and a re-capture confirming the startup-latency reduction. — NOW ACTIONABLE. Target confirmed: the SpamBayes engine deserialization (~67.5 s cold) awaited inside the sequential `Engines` startup phase. Candidate fixes: (1) move engine init (at least Spam) off the sequential startup critical path via the existing `IdleAsyncQueue`/`LoadWhenIdle` so startup completion is not gated by the load; and/or (2) reduce the SpamBayes deserialization cost (serialization format / cached compact binary form / avoid `PreserveReferencesHandling.All` on a large graph). Fix approach to be settled by the Phase 4 plan.
