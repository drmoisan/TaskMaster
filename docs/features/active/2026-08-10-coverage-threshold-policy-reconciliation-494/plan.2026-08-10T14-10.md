...
- **Out of bounds (owned by `csharp-toolchain-gate-fidelity-512`):** the `CLAUDE.md` C# toolchain command
  blocks (regions 181-208, 377-386, and 397-405 (as of `edf3d34c`; the block's trailing "If any step fails"
  sentence at 404 is inside the region)), `.claude/rules/csharp.md`, and
  `.claude/skills/csharp-qa-gate/SKILL.md`. The anchor-based rule below — each region opens at its own
  heading and ends at the line before the next `##`/`###` heading — is the governing definition; the
  numeric spans are convenience labels only.
...
- [ ] [P0-T3] Verify the upstream dependencies have landed by grepping `scripts/vscode/Invoke-MSTestWithCoverage.Helpers.ps1` for `Get-CoberturaLineConditionCoverageParts` and `Merge-CoberturaClassesByFilename`, and `coverage.config` plus the helpers for the #457 nested-lambda exclusion handling; record the result in `<FEATURE>/evidence/baseline/upstream-dependency-check.<TS>.md`.
  - Acceptance: the artifact states, per symbol, present or absent. **Halt condition:** if `Get-CoberturaLineConditionCoverageParts` is absent, #441/#478 have not landed on this branch; stop and escalate rather than measuring under the defective arithmetic. The artifact must additionally record whether `mcp__drm-copilot__potential_to_issue` is callable in this session. If it is not, record `PROMOTION MCP UNAVAILABLE` and continue; the Phase 5 promotion tasks then follow their recorded-blocker branch.
- [ ] [P0-T4] Execute the D1 execution-time provenance re-verification gate and write `<FEATURE>/evidence/baseline/d1-provenance-reverification.<TS>.md`, running exactly: `git log --follow --oneline -- .claude/rules/quality-tiers.md`; `git log --follow --oneline -- .claude/rules/general-unit-test.md`; `git log -L 23,24:.claude/rules/general-unit-test.md`; `git log -L 31,46:.claude/rules/general-unit-test.md`; `git log -L 292,306:CLAUDE.md`; `git show --stat 48e46387`; `git show --stat --format="" 48e46387 -- CLAUDE.md`. All line spans above are stated **as of `edf3d34c`**. Before running each `git log -L`, re-locate the span by its anchor text — `- **Line coverage must remain >= 85%` (`.claude/rules/general-unit-test.md`, for span `23,24`), `## Coverage Exclusion Policy` (`.claude/rules/general-unit-test.md`, for span `31,46`), and `### UT2. Coverage and Scenarios` (`CLAUDE.md`, for span `292,306`) — and record the substituted span in the artifact alongside the executed command.
...
- [ ] [P0-T5] Verify that `CLAUDE.md` § UT2 is textually disjoint from every 512-owned region, by locating the line span of the anchor `### UT2. Coverage and Scenarios` through the line before `- **Scenario Completeness**`, and the line spans of the three C# toolchain command blocks (regions 181-208, 377-386, and 397-405 (as of `edf3d34c`; the block's trailing "If any step fails" sentence at 404 is inside the region), re-located by the heading that opens each region: `### C#1. Tooling & Baseline for C#` for 181-208, `### CUT3. C# Toolchain Command Selection` for 377-386, and `## C# Toolchain (run in this exact order)` for 397-405; the anchor-based rule governs — each region ends at the line before the next `##`/`###` heading, so region 397-405 terminates at the line before `## Key Skills Reference` and includes the trailing "If any step fails, fix and restart from step 1." sentence); record all spans in `<FEATURE>/evidence/baseline/claude-md-region-disjointness.<TS>.md`.
...
- [ ] [P0-T7] Capture the pre-change coverage-numeral inventory by grepping the governance surface for `>= 8[05]%|>= 75%|>= 90%|85\.0|75\.0|80\.0|below 8[05]( percent|%)|below 90( percent|%)|8[05] percent|90 percent|0\.8[05]\b|0\.9[05]\b` across `CLAUDE.md`, `AGENTS.md`, `.claude/rules/`, `.claude/skills/`, `.claude/agents/`, `.claude/hooks/`, `.github/instructions/`, `.agents/`, and `.codex/`, and write the full hit list to `<FEATURE>/evidence/baseline/coverage-numeral-inventory.<TS>.md`.
  - Acceptance: the artifact records path plus line plus text for every hit and a total count, records the exact pattern used, and classifies each hit as `numeric-literal` or `prose`. The prose forms known to exist — `.claude/hooks/validate-feature-review-coverage.ps1` ("below 80 percent") and `.claude/agents/feature-review.md` ("repo-wide below 80, new-file below 90") — must appear in the hit list; their absence means the pattern was not applied as written and fails this task. This is the AC1/AC3 pre-state and the AC10 completeness input.
...
- [ ] [P3-T13] Prove AC8 internal consistency by grepping `.claude/hooks/validate-feature-review-coverage.ps1` and `scripts/vscode/Invoke-CoverageThresholdGate.ps1` for coverage numerals and writing `<FEATURE>/evidence/other/hook-consistency-proof.<TS>.md`.
  - Acceptance: exactly one **enforced repository-wide line-floor** numeral is compared against in executable code across both files, and it is `LineFloorPercent` in `Get-CoverageThresholdPolicy`; `NewCodeFloorPercent` and `BranchGated` in that same hashtable are policy constants and are enumerated separately, not counted as a second enforced floor. No other executable statement in either file compares against a coverage literal. The value equals the number stated in `CLAUDE.md` § UT2. The documentation occurrences written by P3-T7 — the repository-wide line floor (`80`) and the new-code line floor (`90`) — are the permitted non-executable occurrences and are listed individually in the artifact with that classification. The `80` written by P3-T7 into the documentation block equals `(Get-CoverageThresholdPolicy).LineFloorPercent`.
...
- [ ] [P4-T1] Create the committed below-threshold fixture `tests/fixtures/coverage/below-threshold.cobertura.xml`, a minimal valid Cobertura document whose root attributes give a repository-wide line rate strictly below the D1 floor (for example `lines-valid="1000" lines-covered="700"`).
  - Acceptance: the file exists at `tests/fixtures/coverage/below-threshold.cobertura.xml`, is valid XML, is **not** matched by `.gitignore` (verified with `git check-ignore -v`, which must exit 1), parses through `Get-CoberturaCoverageRates` to a line rate below 80.0, and is referenced by both P4-T12 and the Pester suite. It is committed by P6-T9.
...
- [ ] [P5-T1] Author the potential-feature entries that the MCP promotion lifecycle consumes: `docs/features/potential/<TS-date>-coverage-numeral-citation-conversion.md` (FU-A), `docs/features/potential/<TS-date>-coverage-measurement-determinism.md` (FU-B, authored only if P1-T5 recorded `TOLERANCE NOT MET`), and `docs/features/potential/<TS-date>-remove-temp-extract-coverage.md` (FU-C).
  - Acceptance: each authored entry exists at a repository-relative path under `docs/features/potential/` and is the exact `potential_path` value the corresponding promotion task (P5-T2, P5-T3, P5-T4) passes to `mcp__drm-copilot__potential_to_issue`. If `mcp__drm-copilot__potential_to_issue` is unavailable (per the P0-T3 probe), author the potential entries anyway, record `PROMOTION MCP UNAVAILABLE` in `<FEATURE>/evidence/issue-updates/promotion-blocker.<TS>.md`, and continue the plan to completion. Do **not** file issues by any other route and do **not** halt. The orchestrator holds the promotion MCP surface and completes the promotion from the recorded potential-entry paths after execution. P5-T2, P5-T3, P5-T4, P5-T16 and P5-T17 then record a `remediation-required` outcome rather than a PASS, and the completion report names FU-A and FU-C as outstanding.
- [ ] [P5-T2] File follow-up issue **FU-A** ("Convert all remaining coverage-numeral sites to citations of `CLAUDE.md` § UT2") through the MCP promotion lifecycle and mirror the result to `<FEATURE>/evidence/issue-updates/fu-a.<TS>.md`.
  - Acceptance: the mirror records the created issue number and URL, and the issue body explicitly names `.claude/agents/feature-review.md` (highest priority, self-contradicting), `.claude/rules/powershell.md`, `.claude/skills/powershell-qa-gate/SKILL.md`, `.claude/skills/feature-review-workflow/SKILL.md`, `.github/instructions/general-unit-test.instructions.md`, `AGENTS.md`, the three divergent `.agents/` files, `.codex/hooks/validate-feature-review-coverage.ps1`, and the `.claude/agent-memory/**` entries superseded on landing. Under the P5-T1 recorded-blocker branch (`PROMOTION MCP UNAVAILABLE` per the P0-T3 probe), the mirror instead records `remediation-required`, the authored FU-A potential-entry path from P5-T1 as the promotion input the orchestrator must consume, and the full issue-body site list above as the text to be filed; the created-issue-number-and-URL requirement is deferred to the orchestrator and is not a PASS condition for this task.
- [ ] [P5-T3] Record the **FU-B** determination from the P1-T5 spread result and, if `TOLERANCE NOT MET`, file FU-B ("Coverage measurement determinism") through the MCP promotion lifecycle; mirror either outcome to `<FEATURE>/evidence/issue-updates/fu-b.<TS>.md`.
  - Acceptance: the mirror records the measured line-rate and lines-valid spreads and states either the created issue number or an explicit `FU-B NOT REQUIRED` determination citing the tolerance result. If P1-T5 recorded `TOLERANCE NOT MET` and the P0-T3 probe recorded `PROMOTION MCP UNAVAILABLE`, the mirror records `FU-B REQUIRED — remediation-required` with the measured line-rate and lines-valid spreads and the authored FU-B potential-entry path, in place of a created issue number. An unrecorded determination fails this task.
- [ ] [P5-T4] File follow-up issue **FU-C** ("Remove or relocate `scripts/temp-extract-coverage.ps1`") through the MCP promotion lifecycle and mirror the result to `<FEATURE>/evidence/issue-updates/fu-c.<TS>.md`.
  - Acceptance: the mirror records the created issue number and URL; `scripts/temp-extract-coverage.ps1` itself is not modified by this feature. Under the P5-T1 recorded-blocker branch, the mirror instead records `remediation-required` and the authored FU-C potential-entry path from P5-T1; the created-issue-number-and-URL requirement is deferred to the orchestrator and is not a PASS condition for this task. `scripts/temp-extract-coverage.ps1` is not modified under either branch.
...
- [ ] [P5-T7] Check off **AC1** in both `docs/features/active/2026-08-10-coverage-threshold-policy-reconciliation-494/spec.md` and `docs/features/active/2026-08-10-coverage-threshold-policy-reconciliation-494/issue.md`, citing `<FEATURE>/evidence/other/authority-single-numeral-proof.<TS>.md`.
  - Acceptance: exactly one acceptance-criterion checkbox is flipped by this task, in both files; the only change made to `issue.md` and `spec.md` is the `- [ ]` → `- [x]` marker (criterion text is preserved byte-for-byte); the evidence pointer is recorded in the task's progress output and in `<FEATURE>/evidence/issue-updates/ac-status-summary.<TS>.md` (P5-T17), not in the criterion text.
...
- [ ] [P5-T10] Check off **AC4** in both `docs/features/active/2026-08-10-coverage-threshold-policy-reconciliation-494/spec.md` and `docs/features/active/2026-08-10-coverage-threshold-policy-reconciliation-494/issue.md`, citing `<FEATURE>/evidence/regression-testing/ac4-negative-path-proof.<TS>.md`.
  - Acceptance: exactly one acceptance-criterion checkbox is flipped by this task, in both files, and the cited proof names Case A and Case B individually; the only change made to `issue.md` and `spec.md` is the `- [ ]` → `- [x]` marker (criterion text is preserved byte-for-byte); the evidence pointer is recorded in the task's progress output and in `<FEATURE>/evidence/issue-updates/ac-status-summary.<TS>.md` (P5-T17), not in the criterion text.
...
- [ ] [P5-T12] Check off **AC6** in both `docs/features/active/2026-08-10-coverage-threshold-policy-reconciliation-494/spec.md` and `docs/features/active/2026-08-10-coverage-threshold-policy-reconciliation-494/issue.md`, citing the P2-T10, P2-T15, P2-T16 greps and `<FEATURE>/evidence/other/tier-reference-dispositions.<TS>.md`.
  - Acceptance: exactly one acceptance-criterion checkbox is flipped by this task, in both files, and the widened AC6 text is the text being checked off; the only change made to `issue.md` and `spec.md` is the `- [ ]` → `- [x]` marker (criterion text is preserved byte-for-byte); the evidence pointer is recorded in the task's progress output and in `<FEATURE>/evidence/issue-updates/ac-status-summary.<TS>.md` (P5-T17), not in the criterion text.
- [ ] [P5-T13] Check off **AC7** in both `docs/features/active/2026-08-10-coverage-threshold-policy-reconciliation-494/spec.md` and `docs/features/active/2026-08-10-coverage-threshold-policy-reconciliation-494/issue.md`, citing `<FEATURE>/evidence/baseline/coverage-remeasurement-spread.<TS>.md` and `<FEATURE>/evidence/other/d1-measurement-reconciliation.<TS>.md`.
  - Acceptance: exactly one acceptance-criterion checkbox is flipped by this task, in both files, and the cited evidence pre-dates every Phase 2 governance edit; the only change made to `issue.md` and `spec.md` is the `- [ ]` → `- [x]` marker (criterion text is preserved byte-for-byte); the evidence pointer is recorded in the task's progress output and in `<FEATURE>/evidence/issue-updates/ac-status-summary.<TS>.md` (P5-T17), not in the criterion text.
...
- [ ] [P5-T15] Check off **AC9** in both `docs/features/active/2026-08-10-coverage-threshold-policy-reconciliation-494/spec.md` and `docs/features/active/2026-08-10-coverage-threshold-policy-reconciliation-494/issue.md`, citing `<FEATURE>/evidence/other/test-determinism-audit.<TS>.md` and recording that the mirrored test paths follow the `spec.md` Test Strategy restatement.
  - Acceptance: exactly one acceptance-criterion checkbox is flipped by this task, in both files, and the restatement rationale is cited rather than silently deviated from; the only change made to `issue.md` and `spec.md` is the `- [ ]` → `- [x]` marker (criterion text is preserved byte-for-byte); the evidence pointer is recorded in the task's progress output and in `<FEATURE>/evidence/issue-updates/ac-status-summary.<TS>.md` (P5-T17), not in the criterion text.
- [ ] [P5-T16] Check off **AC10** in both `docs/features/active/2026-08-10-coverage-threshold-policy-reconciliation-494/spec.md` and `docs/features/active/2026-08-10-coverage-threshold-policy-reconciliation-494/issue.md`, citing the completed disposition table and the FU-A issue number from `<FEATURE>/evidence/issue-updates/fu-a.<TS>.md`.
  - Acceptance: exactly one acceptance-criterion checkbox is flipped by this task, in both files, and FU-A exists as a real issue rather than as prose (or, under the P5-T1 recorded-blocker branch, this task flips **no** checkbox: the AC10 box is left `- [ ]` in both files, the task records a `remediation-required` outcome rather than a PASS, and the completion report names AC10 as outstanding pending the orchestrator-side FU-A promotion); the only change made to `issue.md` and `spec.md` is the `- [ ]` → `- [x]` marker (criterion text is preserved byte-for-byte); the evidence pointer is recorded in the task's progress output and in `<FEATURE>/evidence/issue-updates/ac-status-summary.<TS>.md` (P5-T17), not in the criterion text.
- [ ] [P5-T17] Write the acceptance-criteria status summary to `<FEATURE>/evidence/issue-updates/ac-status-summary.<TS>.md`, listing AC1-AC10 with status and evidence path.
  - Acceptance: all ten criteria are listed, each with its actual source-file status (`[x]` delivered, or `[ ]` accompanied by a `remediation-required` note naming the blocker) and a resolvable evidence path. A criterion recorded here as `[x]` whose checkbox is not `[x]` in both source files fails this task.
...
- [ ] [P6-T2] Run the final PowerShell lint stage `mcp__drm-copilot__run_poshqc_analyze` and write `<FEATURE>/evidence/qa-gates/final-ps-analyze.<TS>.md`.
  - Acceptance: `EXIT_CODE:` is recorded and is no worse than the exit code in `<FEATURE>/evidence/baseline/ps-analyze.<TS>.md`, and the diagnostic set contains **zero new diagnostics** relative to that baseline, compared rule-by-rule and file-by-file. Any diagnostic introduced by this feature must be fixed and the loop restarted at P6-T1. Pre-existing diagnostics outside the authorized edit path list are recorded and left unchanged; they do not license an `EXIT_CODE: SKIPPED` outcome. If the baseline recorded `EXIT_CODE: 0`, this task requires `EXIT_CODE: 0`.
...
- [ ] [P6-T4] Run the final PowerShell test stage with coverage `mcp__drm-copilot__run_poshqc_test` and write `<FEATURE>/evidence/qa-gates/final-ps-pester-coverage.<TS>.md`.
  - Acceptance: `EXIT_CODE: 0`, all tests pass, and `Output Summary:` records post-change **line coverage %** and **branch coverage %** plus passed/failed/total counts. If the MCP tool is unavailable in the execution session, **or** returns a non-zero/negative exit code without per-test diagnostic detail, this task is authorized to fall back to `pwsh -NoProfile -Command "Invoke-Pester -Path tests -CI -CodeCoverage @('scripts/**/*.ps1','.claude/hooks/*.ps1') -CodeCoverageOutputFile artifacts/pester/powershell-coverage.xml -CodeCoverageOutputFileFormat JaCoCo"`. Both the MCP attempt and the fallback must be recorded with their `EXIT_CODE:`, and the artifact must state which route produced the reported numbers; the `EXIT_CODE: 0` requirement applies to the route that produced the reported figures. `EXIT_CODE: SKIPPED` is not a valid outcome. The JaCoCo artifact `artifacts/pester/powershell-coverage.xml` exists after the run. The artifact must additionally enumerate the executed test **files** and prove both new files were collected with a non-zero test count each — `tests/scripts/vscode/Invoke-CoverageThresholdGate.Tests.ps1` and `tests/.claude/hooks/validate-feature-review-coverage.Tests.ps1`. If either file is absent from the executed inventory, re-run with an explicit `scan_folders` argument naming `tests`, record which route was used, and record whether `config/poshqc-scan.json` had to be authored. A green suite that did not collect both files fails this task.
...
- [ ] [P6-T9] Commit all source, test, governance, evidence, and permitted-incidental (`.claude/agent-memory/**`) changes on `bug/coverage-threshold-policy-reconciliation-494` with a message referencing issue #494, and record the commit sha in `<FEATURE>/evidence/qa-gates/final-commit.<TS>.md`.
  - Acceptance: `git status --porcelain` is empty after the commit; the evidence artifacts under `<FEATURE>/evidence/` are included; the artifact lists the permitted-incidental paths committed, separately from the authorized-scope paths.
