...
- [x] [P0-T1] Read the repository policy documents in the mandated order and record the read list in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/baseline/phase0-instructions-read.md`.
...
- [x] [P0-T2] Record worktree identity and base commit in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/baseline/worktree-identity.md`.
...
- [x] [P0-T3] Bootstrap the .NET SDK and the CSharpier tool manifest for this worktree and record the outcome in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/baseline/toolchain-bootstrap.md`.
...
- [x] [P0-T4] Restore NuGet packages for the non-SDK-style projects and record the outcome in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/baseline/nuget-restore.md`.
...
- [x] [P0-T5] Capture the CSharpier baseline for `UtilitiesCS/Extensions/DfDeedle.cs` and the rest of the tree in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/baseline/csharpier-check-baseline.md`.
...
- [x] [P0-T6] Capture the analyzer build baseline over the solution file in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/baseline/msbuild-analyzers-baseline.md`.
...
- [x] [P0-T7] Capture the nullable build baseline over the solution file in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/baseline/msbuild-nullable-baseline.md`.
...
- [x] [P0-T8] Probe whether the four shell-icon test classes still stall the local testhost, and record the decision in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/baseline/shell-icon-stall-probe.md`.
...
- [x] [P0-T9] Capture the full-suite baseline test run with coverage and write the Cobertura document to `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/baseline/coverage-baseline.cobertura.xml`.
...
- [x] [P0-T10] Extract baseline per-file coverage for the six production write-set paths into `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/baseline/per-file-coverage-baseline.md`.
...
- [x] [P0-T11] Record the baseline line counts of the eleven write-set `.cs` files in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/baseline/line-count-baseline.md`.
...
- [x] [P0-T12] Record the pre-existing 500-line-cap state in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/baseline/line-cap-preexisting.md`.
...
- [x] [P0-T13] Probe cross-assembly log4net capture by temporarily appending a probe test to `UtilitiesCS.Test/Extensions/DfDeedle_COM_Tests.cs` and record the outcome in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/baseline/log4net-capture-probe.md`.
...
- [x] [P0-T14] Remove the probe test from `UtilitiesCS.Test/Extensions/DfDeedle_COM_Tests.cs` and confirm the file is byte-identical to the base commit.
...
- [x] [P0-T15] Probe the plan validator MCP tool and record the outcome in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/baseline/mcp-validator-probe.md`.
...
- [x] [P1-T1] Create `UtilitiesCS/Extensions/DfDeedle.QfcColumns.cs` as a new partial of `public static partial class DfDeedle` in namespace `UtilitiesCS`, holding the four column methods moved verbatim out of `UtilitiesCS/Extensions/DfDeedle.cs`.
...
- [x] [P1-T2] Remove the four moved methods from `UtilitiesCS/Extensions/DfDeedle.cs` so no member is declared twice.
...
- [x] [P1-T3] Widen `AddQfcColumnsAsync` in `UtilitiesCS/Extensions/DfDeedle.QfcColumns.cs` to `internal` and add the two optional parameters, keeping the existing defective timeout loop unchanged.
...
- [x] [P1-T4] Add the not-yet-implemented validator `ValidateRequiredEmailColumns` to `UtilitiesCS/Extensions/DfDeedle.QfcColumns.cs` with a body that performs no validation.
...
- [x] [P1-T5] Create `TaskMaster/Ribbon/RibbonCommandBoundary.cs` as an `internal sealed class` in namespace `TaskMaster` that is not marked `[ExcludeFromCodeCoverage]`, with a pass-through `RunAsync` that does not catch.
...
- [x] [P1-T6] Add the `<Compile Include>` entry for the new partial to `UtilitiesCS/UtilitiesCS.csproj` adjacent to the existing entry for the sibling partial.
...
- [x] [P1-T7] Add the `<Compile Include>` entry for the ribbon boundary to `TaskMaster/TaskMaster.csproj` adjacent to the existing ribbon-runner entry.
...
- [x] [P1-T8] Create the three new test files as compiling stubs: `UtilitiesCS.Test/Extensions/DfDeedleQfcColumnTimeoutTests.cs`, `UtilitiesCS.Test/Extensions/DfDeedleRequiredColumnValidationTests.cs`, and `TaskMaster.Test/Ribbon/RibbonCommandBoundaryTests.cs`.
...
- [x] [P1-T9] Create `QuickFiler.Test/Controllers/QfcDatamodelRethrowTests.cs` as a compiling stub in namespace `QuickFiler.Controllers.Tests`.
  - Acceptance: the file exists and contains the literal `[TestClass]`.

- [x] [P1-T10] Add the four `<Compile Include>` entries for the new test files to `UtilitiesCS.Test/UtilitiesCS.Test.csproj`, `TaskMaster.Test/TaskMaster.Test.csproj`, and `QuickFiler.Test/QuickFiler.Test.csproj`.
...
- [x] [P1-T11] Repair the two reflection invocations in `UtilitiesCS.Test/Extensions/DfDeedle_COM_Tests.cs` that break against the widened signature.
...
- [x] [P1-T12] Format the tree with `dotnet tool run csharpier format .` and verify with `dotnet tool run csharpier check .`, recording both in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/qa-gates/p1-csharpier.md`.
...
- [x] [P1-T13] Build and run the three affected test assemblies to confirm the seams compile and no existing test regressed, recording the result in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/qa-gates/p1-seam-build-and-scoped-tests.md`.
...
- [x] [P2-T1] [expect-fail] Add the AC1 non-overlap test `AddQfcColumnsAsync_ThreeDeadlines_InvokesColumnAdderExactlyOnce` to `UtilitiesCS.Test/Extensions/DfDeedleQfcColumnTimeoutTests.cs`.
...
- [x] [P2-T2] [expect-fail] Add the AC1 loud-failure test `AddQfcColumnsAsync_ThirdDeadlineExpires_ThrowsNamingFolderAndStep` to `UtilitiesCS.Test/Extensions/DfDeedleQfcColumnTimeoutTests.cs`.
...
- [x] [P2-T3] [expect-fail] Add the three AC1 positive-path tests to `UtilitiesCS.Test/Extensions/DfDeedleQfcColumnTimeoutTests.cs`, one per deadline on which the adder completes.
...
- [x] [P2-T4] Add the AC1 cancellation guard test `AddQfcColumnsAsync_CancellationRequestedMidLoop_DoesNotThrowTimeoutExhausted` to `UtilitiesCS.Test/Extensions/DfDeedleQfcColumnTimeoutTests.cs`.
...
- [x] [P2-T5] [expect-fail] Add the AC2 timing tests to `UtilitiesCS.Test/Extensions/DfDeedleQfcColumnTimeoutTests.cs`, using the assertion strategy selected by the P0-T13 verdict.
  ...
  - **AC2 assertion strategy actually implemented — a third strategy, recorded per AC14.** Neither of the two branches above was implemented as written. Phase 3 established that the P0-T13 verdict `LOG4NET_CROSS_ASSEMBLY_CAPTURE: CONFIRMED` is not reproducible: neither `UtilitiesCS` nor `UtilitiesCS.Test` carries a log4net `XmlConfigurator` attribute, so the default repository is unconfigured, `Hierarchy.IsDisabled` reports every level disabled, and the production `Debug` call is a no-op. Both AC2 tests therefore captured zero events whether or not the instrumentation was present. The `CONFIRMED` branch alone captures nothing for that reason, and the `NOT AVAILABLE` branch does not help either, because it addresses a cross-assembly repository mismatch while the actual obstruction is a repository-level disable. The strategy implemented keeps the `CONFIRMED` branch's appender attachment and existence assertion and additionally makes the repository emit at all: the helper `CaptureDfDeedleLog` sets `Configured` to true on the `Hierarchy` and calls `ActivateOptions()` on the `MemoryAppender` for the duration of the capture, restoring the previous `Configured` value and the logger's previous `Level` in a `finally`. The class carries `[DoNotParallelize]`, which bounds that process-wide mutation within the class. The fallback floated in spec.md's assumptions section, asserting AC2 indirectly through the injected column-adder, remains unused for the reason P0-T13 records. Correction A1 amends the P0-T13 artifact, and correction A2 re-derives the AC2 fail-before evidence against the repaired capture, because the original fail-before run did not discriminate.
...
- [x] [P2-T6] [expect-fail] Add the AC3 negative tests to `UtilitiesCS.Test/Extensions/DfDeedleRequiredColumnValidationTests.cs`, one per required key removed in turn.
...
- [x] [P2-T7] [expect-fail] Add the AC3 message-content, multi-missing, case-variant and positive tests to `UtilitiesCS.Test/Extensions/DfDeedleRequiredColumnValidationTests.cs`.
...
- [x] [P2-T8] [expect-fail] Add the AC4 stack-preservation test `GetEmailsInViewDfAsync_InnerFailure_PreservesOriginatingFrameInStack` to `QuickFiler.Test/Controllers/QfcDatamodelRethrowTests.cs`.
...
- [x] [P2-T9] [expect-fail] Add the AC5 boundary behaviour tests to `TaskMaster.Test/Ribbon/RibbonCommandBoundaryTests.cs`.
...
- [x] [P2-T10] [expect-fail] Add the AC5 handler shape pin `NamedQuickFilerHandlers_AreAwaitedAsyncVoidAndRouteThroughTheBoundary` to `TaskMaster.Test/Ribbon/RibbonCommandBoundaryTests.cs`.
...
- [x] [P2-T11] Run the three affected assemblies and record the consolidated fail-before set in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/regression-testing/p2-consolidated-fail-before.md`.
...
- [x] [P3-T1] Replace the recursive timeout loop in `UtilitiesCS/Extensions/DfDeedle.QfcColumns.cs` with a single held task re-deadlined up to three times.
...
- [x] [P3-T2] Add the descriptive timeout-exhausted throw to `UtilitiesCS/Extensions/DfDeedle.QfcColumns.cs` so the method fails loudly after its final deadline.
...
- [x] [P3-T3] Add the AC2 timing instrumentation around the property enumeration in `UtilitiesCS/Extensions/DfDeedle.QfcColumns.cs`.
...
- [x] [P3-T4] Add the AC2 timing instrumentation around each of the six column operations in `UtilitiesCS/Extensions/DfDeedle.QfcColumns.cs`.
...
- [x] [P3-T5] Confirm no banned time-abstraction symbol was introduced into `UtilitiesCS/Extensions/DfDeedle.QfcColumns.cs` and that no `TimeoutAfter` overload was added or edited.
...
- [x] [P3-T6] Run the analyzer build and the three affected assemblies, recording the AC1 and AC2 green transition in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/regression-testing/p3-ac1-ac2-pass-after.md`.
...
- [x] [P4-T1] Implement `ValidateRequiredEmailColumns` in `UtilitiesCS/Extensions/DfDeedle.QfcColumns.cs` as a pure ordinal validator.
...
- [x] [P4-T2] Call the validator from the asynchronous entry point in `UtilitiesCS/Extensions/DfDeedle.cs`.
...
- [x] [P4-T3] Call the validator from the synchronous entry point in `UtilitiesCS/Extensions/DfDeedle.cs`, closing the duplicate unchecked indexing on that path.
...
- [x] [P4-T4] Run the toolchain steps and the three affected assemblies, recording the AC3 green transition in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/regression-testing/p4-ac3-pass-after.md`.
...
- [x] [P5-T1] Change the rethrow at line 108 of `QuickFiler/Controllers/QfcDatamodel.FrameBuilding.cs` from `throw e;` to `throw;`.
...
- [x] [P5-T2] Change the two identical sibling rethrows at lines 359 and 400 of `QuickFiler/Controllers/QfcDatamodel.cs` from `throw e;` to `throw;`.
...
- [x] [P5-T3] Confirm the two out-of-scope `throw e;` occurrences outside the `QfcDatamodel` partial family are unchanged, recording the check in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/qa-gates/p5-ac10-out-of-scope-throws.md`.
...
- [x] [P5-T4] Run the toolchain steps and the three affected assemblies, recording the AC4 green transition in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/regression-testing/p5-ac4-pass-after.md`.
...
- [x] [P6-T1] Implement the catching boundary in `TaskMaster/Ribbon/RibbonCommandBoundary.cs`.
...
- [x] [P6-T2] Render inner exception detail in the message that `TaskMaster/Ribbon/RibbonCommandBoundary.cs` passes to the presentation sink.
...
- [x] [P6-T3] Add the boundary field and the failure sink to `TaskMaster/Ribbon/RibbonViewer.cs`, modelled on the existing static failure reporter at lines 311 to 315.
...
- [x] [P6-T4] Route the three named handlers in `TaskMaster/Ribbon/RibbonViewer.cs` through the boundary, leaving every other member untouched.
...
- [x] [P6-T5] Run the toolchain steps and the three affected assemblies, recording the AC5 green transition in `docs/features/active/2026-09-06-quickfiler-crash-column-add-timeout-swallowed-keynotfound-798/evidence/regression-testing/p6-ac5-pass-after.md`.
