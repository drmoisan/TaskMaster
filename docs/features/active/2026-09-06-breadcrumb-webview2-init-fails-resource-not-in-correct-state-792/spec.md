...
- No outstanding prerequisite. The audit confirming that the breadcrumb document does not depend on persisted browsing storage has been performed and is recorded in section 6 of the research record, so the options direction is already final. This is a discharged audit, not scoped work. See Risks.
...
1. **Options direction.** All three sites converge on `--incognito `. Two of three already use it and the item-body preview has always used it. The confirmation that the breadcrumb document does not depend on persisted browsing storage is complete, so this direction is final rather than provisional: an audit of the three files that generate the document found no use of any browser storage API — zero matches for `localStorage`, `sessionStorage`, `indexedDB`, `document.cookie`, `navigator.storage`, `caches.`, `openDatabase` — and no external resource reference for the browser to cache.
...
- The breadcrumb document, which is generated locally and delivered by navigating to a string, does not depend on persisted browsing storage. This is an established fact rather than an open assumption: the audit recorded in section 6 of the research record found the document uses no browser storage API at all and references no external resource.
...
- **CLOSED — the breadcrumb document does not depend on persisted browsing storage.** The audit previously carried here as a prerequisite has been performed and is recorded in section 6 of the research record. Audited: the three files that generate the document, which are BreadcrumbDocumentAssets.cs, BreadcrumbHtmlRenderer.cs and BreadcrumbRowBuilder.cs under UtilitiesCS/OutlookObjects/Folder. Found: every stylesheet and script in the document is a compile-time `const` string emitted inline, so there is no external resource for the browser to fetch or cache; a scoped search of those three files for `localStorage`, `sessionStorage`, `indexedDB`, `document.cookie`, `navigator.storage`, `caches.`, `openDatabase`, `<link`, `<img`, `script src`, `fetch(`, `XMLHttpRequest` and `serviceWorker` returns zero matches; the document's only host-communication surface is `window.chrome.webview.postMessage` outbound and `window.chrome.webview.addEventListener('message', ...)` inbound, which is the WebView2 host bridge rather than browser storage and is not restricted by incognito; and all display state arrives from .NET through the `render` and `subfolderResult` messages and is re-pushed on each initialization, so nothing is restored from browser storage. Incognito restricts persistent storage and this document uses none, so converging the breadcrumb host onto `--incognito ` cannot break it. Consequence: the alternative convergence direction (no additional arguments at all three sites) is not taken. That alternative would additionally have undone issue #463's deliberate privacy fix on the two item-preview sites, which navigate untrusted message bodies; that is a separate and independent reason against it, and it is not what closes the storage question.
