...
### Remediation Cycle 1 — 2026-08-08T14-26

Executed against `remediation-plan.2026-08-08T14-26.md` from `remediation-inputs.2026-08-08T14-26.md`, which entered the cycle with **zero blocking findings** and exactly two in-scope Medium findings. This cycle changed **no acceptance criterion's state**; every automated criterion was already checked off by the implementation cycle, and F1 and F2 are quality defects in delivered work rather than unmet criteria.

**F1 — the AC5 ribbon-XML assertion was made non-vacuous. Remediated and proven.** `RibbonExplorerXml_EveryEngineBackedControlDeclaresGetEnabledCallback` in `TaskMaster.Test\Ribbon\RibbonExplorerXmlTests.cs` previously read `...Attribute("getEnabled")?.Value.Should().Be(...)`, where the null-conditional operator short-circuited the whole chain — including `.Should()` — whenever the attribute was absent, which is precisely the regression the test names. The attribute is now bound to a local and asserted `NotBeNull` before `Value` is dereferenced, so all three required failure conditions (attribute missing, attribute present with the wrong value, attribute present but empty) reach a real assertion. Non-vacuity was **demonstrated, not asserted**, by a recorded mutate-build-fail-restore cycle: one `getEnabled` attribute was temporarily deleted from the embedded resource, the assembly was rebuilt and the mutated byte content confirmed at seven attributes before any test ran, the test was recorded **Failed** with the verbatim message `Expected getEnabled not to be <null> because control 'TrainSpam' is engine-backed and must declare a getEnabled callback`, and the mutation was then restored and the test recorded **Passed**. Evidence: `evidence/regression-testing/f1-assertion-shape.2026-08-08T14-52.md`, `evidence/regression-testing/f1-mutated-assembly.2026-08-08T14-52.md`, `evidence/regression-testing/f1-fail-proof.2026-08-08T14-52.md`, `evidence/regression-testing/f1-mutation-restored.2026-08-08T14-52.md`, `evidence/regression-testing/f1-pass-after-restore.2026-08-08T14-52.md`. The permanent tree retains no part of the mutation.

**F2 — `RibbonExplorer.xml` line reduction is NOT remediable as specified. Escalated, not fixed.** F2 asked that the three `TriageSet*` `<button>` elements be restored to single-line form while retaining their `getEnabled` attribute, bringing the file to 527 lines or fewer. That edit was made, passed every Phase 2 gate at 524 lines, and was then **reverted** when the repo-wide `csharpier check .` gate rejected it. Measured cause: CSharpier **1.3.0 formats XML**, `.csharpierignore` excludes `*.csproj`/`*.props`/`*.targets` but not `*.xml`, and the default print width is 100 columns. The merge-base single-line form is 78 characters and fits; adding the functionally required `getEnabled="EngineCommand_GetEnabled"` makes it **116 characters**, so the formatter mandates the multi-line form. The 12 lines the review characterised as "incidental churn with no functional purpose" are in fact formatter-mandated, and the 527-line target is unreachable while the mandatory format gate must pass. `RibbonExplorer.xml` therefore **remains at 539 lines and takes a zero-line diff from this cycle**, superseding nothing; the AC25 pre-existing 519-line exception is unchanged in kind. The only viable route to a smaller file is splitting the resource, which AC25 already declines and which belongs to its own issue. Full measured analysis and the rejected alternatives (all of which would have required weakening a gate or widening scope): `evidence/qa-gates/f2-formatter-conflict.2026-08-08T14-52.md`.

**Toolchain.** The four C# gates passed in a single uninterrupted pass on the second Phase 3 attempt (the first was aborted at the format-verification step by the F2 conflict above, and the restart is disclosed rather than omitted): CSharpier format and repo-wide check exit 0 over 1498 files, analyzer build exit 0 with six warnings all matching the pre-existing baseline, nullable build exit 0, and 6338 of 6338 tests passed with zero failed and zero skipped. First-party coverage moved up on an identical denominator — line 85.8462% to **85.8561%**, branch 79.2559% to **79.2702%** — with the `TaskMaster` package LINE counter byte-identical at `missed=1464 covered=3515` across all three measurement points, confirming no production line changed coverage state. Evidence: `evidence/qa-gates/toolchain-clean-pass.2026-08-08T14-52.md` and `evidence/qa-gates/coverage-comparison.2026-08-08T14-52.md`.

**Protected paths.** `TaskMaster\AppGlobals\AppItemEngines.cs`, `UtilitiesCS\Interfaces\IGlobals\IAppItemEngines.cs`, and `TaskMaster\AppGlobals\ApplicationGlobals.cs` each retain a zero-line diff against the merge-base, verified against the working tree before the commit and against the committed tree after it. AC19, AC20, and AC21 remain unchecked and MANUAL-ONLY.
