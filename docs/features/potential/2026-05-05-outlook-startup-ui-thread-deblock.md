# outlook-startup-ui-thread-deblock (Potential Bug)
...
- [ ] Validate a phased startup design where only COM-bound segments stay on the UI thread and background-safe config/file work is explicitly offloaded
