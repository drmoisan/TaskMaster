    ...
    public class FolderWrapperNameAndParentNameComparer : IEqualityComparer<TreeNode<FolderWrapper>>
    {
        public bool Equals(TreeNode<FolderWrapper>? x, TreeNode<FolderWrapper>? y)
        {
            ...
            if (x is null || y is null)
                return false;
            var nameComparison = string.Equals(
                x.Value.Name,
                y.Value.Name,
            ...
            {
                return false;
            } // One has a parent, the other does not
            else if (x.Parent.Value is null && y.Parent.Value is null)
            {
                return true;
            } // Both parents are nodes with null values
            else if (x.Parent.Value is null || y.Parent.Value is null)
            {
                return false;
            } // One parent is a node with null value, the other is not
            else if (x.Parent.Value.Name.IsNullOrEmpty() && y.Parent.Value.Name.IsNullOrEmpty())
            {
                return true;
            } // Both parents have null names
            else if (x.Parent.Value.Name.IsNullOrEmpty() || y.Parent.Value.Name.IsNullOrEmpty())
            {
                return false;
            } // One parent has a null name, the other does not
            else
            {
