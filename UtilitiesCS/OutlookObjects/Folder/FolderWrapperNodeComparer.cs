    ...
    public class FolderWrapperNodeComparer : IEqualityComparer<TreeNode<FolderWrapper>>
    {
        public bool Equals(TreeNode<FolderWrapper>? x, TreeNode<FolderWrapper>? y)
        {
            ...
            if (!comparer.Equals(x.Value, y.Value))
                return false; // If FolderWrapper values are not equal, return false
            ...
            if ((x.Children?.Count ?? 0) == 0 && (y.Children?.Count ?? 0) == 0)
            {
                return true;
            } // Neither has children
            else if ((x.Children?.Count ?? 0) != (y.Children?.Count ?? 0))
            {
                return false;
        ...
        public int GetHashCode(TreeNode<FolderWrapper> obj)
        {
            ...
            obj.Children?.Select(c => c.Value)
                ?.ForEach(wrapper => hashCodes.Add(comparer.GetHashCode(wrapper)));
