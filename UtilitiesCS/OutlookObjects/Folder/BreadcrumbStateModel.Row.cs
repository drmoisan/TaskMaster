...
namespace UtilitiesCS.OutlookObjects.Folder
{
    ...
    public sealed class BreadcrumbStateRow
    {
        ...
        /// <summary>
        /// #440 selected-node state: the index within <see cref="Chain"/> of the node the tree
        /// transitions operate on. A suggestion row is leaf-anchored until a non-leaf segment is
        /// explicitly activated; plain rows carry no node.
        /// </summary>
        public int? ActiveSegmentIndex =>
            IsSuggestion ? _activeSegmentIndex ?? Chain.Count - 1 : (int?)null;

        /// <summary>The currently selected node, or null for a plain row.</summary>
        public FolderBreadcrumbSegment? ActiveSegment =>
            ActiveSegmentIndex.HasValue ? Chain[ActiveSegmentIndex.Value] : null;

        /// <summary>
        /// True when the ACTIVE node carries the expansion affordance. Identical to
        /// <see cref="LeafHasSubfolders"/> while the row is leaf-anchored (#440).
        /// </summary>
        public bool ActiveSegmentHasSubfolders =>
            IsSuggestion && CollapsedAfterIndex == null && ActiveSegment?.HasChildren == true;

        /// <summary>
        /// Selects the non-leaf node at <paramref name="segmentIndex"/> (#440). Refuses plain
        /// rows, a negative index, the leaf index — the leaf is where selection starts, so a
        /// downward move is expressed by child descent instead — and the already-selected index.
        /// A successful activation closes any open expansion, as the fetched subfolders belonged
        /// to the previous node.
        /// </summary>
        /// <returns>True when the selected node changed.</returns>
        public bool ActivateSegment(int segmentIndex)
        {
            if (
                !IsSuggestion
                || segmentIndex < 0
                || segmentIndex >= Chain.Count - 1
        ...
        /// <summary>
        /// Returns the fetched child at <paramref name="childIndex"/> of the open expansion, or
        /// null when the request is not valid (#440).
        /// </summary>
        public FolderBreadcrumbSegment? GetActiveChild(int childIndex)
        {
            if (!IsSuggestion || !LeafExpanded || childIndex < 0 || childIndex >= Subfolders.Count)
            {
                return null;
            }

            return Subfolders[childIndex];
        }

        /// <summary>
        /// Opens the expansion of the ACTIVE node when its affordance is available (#440);
        /// returns false (no-op by contract) so the caller can fall through to legacy behavior.
        /// </summary>
        public bool TryExpandActiveSegment()
        {
            if (!ActiveSegmentHasSubfolders || LeafExpanded)
            {
                return false;
            }
