...
namespace UtilitiesCS.EmailIntelligence
{
    public class AttachmentHelper
    {
        ...
        public static async Task<AttachmentHelper> CreateAsync(
            Attachment attachment,
            DateTime sentOn,
            string saveFolderPath,
            string deleteFolderPath
        ...
        )
        {
            ...
            if (CheckParameters(attachment, sentOn, saveFolderPath, deleteFolderPath))
            {
                ...
                {
                    (AttachmentInfo.FilenameSeed, AttachmentInfo.FileExtension) =
                        GetAttachmentFilename(Attachment);
                }

                if (!AttachmentInfo.FilenameSeed.IsNullOrEmpty())
                {
                    AttachmentInfo.FilenameSeed = FolderConverter.SanitizeFilename(
                        AttachmentInfo.FilenameSeed
                    );
                }
                ...
                AttachmentInfo.FilenameSeed = PrependDatePrefix(
                    AttachmentInfo.FilenameSeed,
                    sentOn
                );
                FilePathSave = AdjustForMaxPath(
                    saveFolderPath,
                    AttachmentInfo.FilenameSeed,
                    AttachmentInfo.FileExtension
                );
                FilePathSaveAlt = AdjustForMaxPath(
                    saveFolderPath,
                    AttachmentInfo.FilenameSeed,
                    AttachmentInfo.FileExtension,
                    GetNameSuffix()
                ...
                if (deleteFolderPath is not null)
                {
                    ...
                    _filePathDelete = AdjustForMaxPath(
                        deleteFolderPath,
                        AttachmentInfo.FilenameSeed,
                        AttachmentInfo.FileExtension
                    );
        ...
        protected internal async Task InitAsync(
            Attachment attachment,
            DateTime sentOn,
            string saveFolderPath,
            string deleteFolderPath
        ...
        {
            get => FilePathHelperSave.FilePath;
            set => FilePathHelperSave.FilePath = value;
        }

        //private string _filePathSaveAlt;
        public string FilePathSaveAlt
        {
            get => FilePathHelperSaveAlt.FilePath;
        ...
        {
            get => FilePathHelperSave.FolderPath;
        ...
        internal bool CheckParameters(Attachment attachment, DateTime sentOn, string saveFolderPath)
        {
            ...
            {
                _errorMessages.Add("The attachment is null");
            }

            if (saveFolderPath.Length >= MAX_PATH - 10)
            {
                _errorMessages.Add(
                    $"The path {saveFolderPath} is too long to save. It must be smaller than {MAX_PATH - 10} characters"
                );
            }
        ...
        )
        {
            ...
            {
                _errorMessages.Add(
                    $"The path {deleteFolderPath} is too long to save. It must be smaller than {MAX_PATH} characters"
                );
            }
        ...
        internal virtual (string Filename, string Extension) GetAttachmentFilename(
            Attachment attachment
        )
        {
            string filename = "";
            ...
            {
                logger.Error(
                    $"Error getting filename for item. Attachment type {attachment.Type}\n{e.Message}",
                    e
                );
            }
