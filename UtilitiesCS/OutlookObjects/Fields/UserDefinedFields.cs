...
namespace UtilitiesCS.OutlookExtensions
{
    public static class UserDefinedFields
    {
        ...
        public static void DeleteUdf(this MailItem item, string fieldName)
        {
            ...
            var idx = item
                .UserProperties.Cast<UserProperty>()
                .ToList()
                .FindIndex(x => x.Name == fieldName);
        ...
        private static Dictionary<OlUserPropertyType, Type> udfTypeLookup = new Dictionary<
            OlUserPropertyType,
            Type
        >
        {
        ...
        private static Dictionary<OlUserPropertyType, TypeGroup> udfGroupLookup = new Dictionary<
            OlUserPropertyType,
            TypeGroup
        >
        {
        ...
        internal static bool ValidPropertyArgs(object value, OlUserPropertyType olUdfType)
        {
            ...
            {
                string msg =
                    $"Argument {nameof(value)}={value} is of type {valueType} "
        ...
        public static UserProperty GetUdf(this object item, string fieldName)
        {
            if (item is MailItem)
            {
                return ((MailItem)item).UserProperties.Find(fieldName);
            }
            else if (item is TaskItem)
            {
                return ((TaskItem)item).UserProperties.Find(fieldName);
            }
            else if (item is AppointmentItem)
            {
                return ((AppointmentItem)item).UserProperties.Find(fieldName);
            }
            else if (item is MeetingItem)
            {
                return ((MeetingItem)item).UserProperties.Find(fieldName);
            }
            else
            {
                throw new ArgumentException(
                    "Unsupported type. Extension defined for MailItem, "
                        + "TaskItem, AppointmentItem, and MeetingItem. "
                        + $"{nameof(item)} is of type {item.GetType().Name}",
                    nameof(item)
                );
        ...
        )
        {
            ...
            catch (System.Exception ex)
            {
                ...
                logger.Error(
                    $"Callstack {TraceUtility.GetMethodTraceString(item, udfName, value, olUdfType)}"
                );
