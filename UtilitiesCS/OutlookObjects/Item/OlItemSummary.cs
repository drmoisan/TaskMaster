...
namespace UtilitiesCS
{
    ...
    public static class OlItemSummary
    {
        ...
        public static string Extract(object Item, Details Flags)
        {
            ...
            {
                return ExtractSummary((AppointmentItem)Item).ToString(Flags);
            }
            else if (Item is MailItem)
            {
                return ExtractSummary((MailItem)Item).ToString(Flags);
            }
            else if (Item is MeetingItem)
            {
                return ExtractSummary((MeetingItem)Item).ToString(Flags);
            }
            else if (Item is TaskRequestItem)
            {
                return ExtractSummary((TaskRequestItem)Item).ToString(Flags);
            }
            else if (Item is TaskRequestUpdateItem)
            {
                return ExtractSummary((TaskRequestUpdateItem)Item).ToString(Flags);
            }
            else
            {
                return $"Details.Type: {Item.GetType().ToString()}";
            }
        }
        ...
        public static string ToString(this Dictionary<Details, string> Dict, Details Flags)
        {
            return String.Join(
                ", ",
                Dict.Where(x => Flags.HasFlag(x.Key)).Select(x => $"{x.Key}: {x.Value}")
            );
        }
        ...
        public static Dictionary<Details, string> ExtractSummary(object Item)
        {
            if (Item is AppointmentItem)
            {
                return ExtractSummary((AppointmentItem)Item);
            }
            else if (Item is MailItem)
            {
                return ExtractSummary((MailItem)Item);
            }
            else if (Item is MeetingItem)
            {
                return ExtractSummary((MeetingItem)Item);
            }
            else if (Item is TaskRequestItem)
            {
                return ExtractSummary((TaskRequestItem)Item);
            }
            else if (Item is TaskRequestUpdateItem)
            {
                return ExtractSummary((TaskRequestUpdateItem)Item);
            }
            else
            {
                throw new ArgumentException($"{Item.GetType().ToString()} is an unsupported type");
            }
            //return $"Type: {item.GetType()} Subject: {item.Subject} StartsOn: {item.Start.ToString("MM-dd-yyyy h:mm t")}";
        }
