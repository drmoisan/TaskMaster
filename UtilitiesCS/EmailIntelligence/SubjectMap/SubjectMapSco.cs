...
namespace UtilitiesCS
{
    ...
    public partial class SubjectMapSco : ConcurrentObservableCollection<SubjectMapEntry>
    {
        ...
        public SubjectMapSco(ISerializableList<string> commonWords)
            : base()
        {
            _commonWords = commonWords;
        }

        public SubjectMapSco(List<SubjectMapEntry> listOfT, ISerializableList<string> commonWords)
            : base(listOfT)
        {
            _commonWords = commonWords;
        }
        ...
        public SubjectMapSco(
            IEnumerable<SubjectMapEntry> IEnumerableOfT,
        ...
        {
            base.ToList().AsParallel().ForAll(entry => entry.Encode(encoder, _tokenizerRegex));
        }
        ...
        public void Add(string subject, string folderName)
        {
            ...
            if (idx == -1)
            {
                try
                {
                    var sme = new SubjectMapEntry(
                        emailFolder: folderName,
                ...
                {
                    logger.Error(
                        $"Error adding {nameof(SubjectMapEntry)}. Skipping entry. {e.Message}"
                    );
                }
