...
namespace UtilitiesCS.EmailIntelligence.FolderRemap
{
    public class FolderRemapController
    {
        ...
        {
            var treeMappings = RemapTree.Roots.SelectMany(x =>
                x.FlattenIf(mapping => mapping.MappedTo is not null)
            );
        ...
        {
            RemapTree
                .Roots.SelectMany(root => root.FindAll(node => node.Depth < level))
            ...
            if (addChecked)
            {
                RemapTree
                    .Roots.SelectMany(root =>
                        root.FindAll(node =>
                            node.FindAll(node => node.Value.MappedTo != null).Any()
                        )
                    )
                    .ForEach(x => _viewer.TlvOriginal.Expand(x));
            }
        ...
        internal CheckStatePutterDelegate PutMappedCheckedState = delegate(
            object rowObject,
            CheckState newValue
        )
        {
        ...
        internal CheckStatePutterDelegate MakeCheckedStatePutter()
        {
            CheckStatePutterDelegate putter = delegate(object rowObject, CheckState newValue)
            {
                ...
                if (newValue == CheckState.Checked)
                {
                    ...
                    if (destination.MappedTo is not null)
                    {
                        destination = destination.MappedTo;
                    }
