...
namespace UtilitiesCS.EmailIntelligence
{
    public abstract class TristateEngine()
    {
        ...
        /// </summary>
        public Func<object, string[]>? Tokenize
        {
            get => _tokenize;
            set => _tokenize = value;
        }
        ...
        /// </summary>
        public Func<object, Task<string[]>>? TokenizeAsync
        {
            get => _tokenizeAsync;
        ...
        /// </summary>
        public Func<string[], double>? CalculateProbability
        {
            get => _calculateProbability;
            set => _calculateProbability = value;
        }
        ...
        /// </summary>
        public Func<string[], Task<double>>? CalculateProbabilityAsync
        {
            get => _calculateProbabilityAsync;
            set => _calculateProbabilityAsync = value;
        }
        ...
        /// probability is inconclusive.
        /// </summary>
        public Func<double, Task<bool?>>? GetTristateAsync
        {
            get => _getTristateAsync;
            set => _getTristateAsync = value;
        }
        ...
        public Func<object, bool, Task>? CallbackAsync
        {
            get => _callbackAsync;
        ...
        public TristateThreshhold? Threshhold
        {
            get => _threshhold;
        ...
        {
            Tokenize.ThrowIfNull(
                $"{nameof(Tokenize)} delegate function cannot be null to Train classifier"
            );
            item.ThrowIfNull($"{nameof(item)} cannot be null to Train classifier");
            // Tokenize.ThrowIfNull above guarantees non-null (the #363 contract does not narrow).
        ...
        public async Task TrainAsync(object item, bool state)
        {
            ...
            item.ThrowIfNull($"{nameof(item)} cannot be null to Train classifier");
            // TokenizeAsync.ThrowIfNull above guarantees non-null (the #363 contract does not narrow).
            var tokens = await TokenizeAsync!(item);
            await TrainAsync(tokens, state);
            if (CallbackAsync is not null)
            {
                await CallbackAsync(item, state);
            }
        }
        ...
        public bool? GetTristate(double probability)
        {
            ...
            // Threshhold.ThrowIfNull above guarantees non-null (the #363 contract does not narrow).
            if (probability > Threshhold!.MinimumTrue)
                return true;
            else if (probability < Threshhold!.MaximumFalse)
                return false;
            else
                return null;
        }
