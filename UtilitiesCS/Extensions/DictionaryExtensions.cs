...
#nullable enable
    ...
    public static class DictionaryExtensions
    {
        ...
        )
        {
            ...
            IEnumerable<IGrouping<TKey, KeyValuePair<TKey, TValue>>> duplicateKVPsByKey = source
                .GroupBy(kvp => kvp.Key)
                .Where(g => g.Count() > 1);
            if (duplicateKVPsByKey.Any())
                throw new InvalidOperationException(
                    $"Cannot convert to dictionary with duplicate keys: {string.Join(",", duplicateKVPsByKey)}"
                );
        ...
        public static Dictionary<TKey, TValue> ToDictionary<TKey, TValue>(
            this IEnumerable<DictionaryEntry> entries
        )
        {
            return entries
        ...
        public static SortedDictionary<K, V> ToSortedDictionary<K, V>(
            this Dictionary<K, V> existing
        )
        {
            return new SortedDictionary<K, V>(existing);
        }

        public static SortedDictionary<string, bool> SearchSortedDictKeys(
            SortedDictionary<string, bool> source_dict,
            string search_string
        )
        {
            var filtered_cats = (
                from x in source_dict
        ...
        public static bool TryAddValues<TKey, TValue>(
        ...
        public static bool TrySubtractValues<TKey, TValue>(
            this ConcurrentDictionary<TKey, TValue> dictionary,
            TKey key,
            TValue value
        )
        ...
        public static bool TryOperateValues<TKey, TValue>(
            this ConcurrentDictionary<TKey, TValue> dictionary,
            TKey key,
            TValue value,
            ...
            while (dictionary.TryGetValue(key, out value))
            {
                ...
                else
                {
                    ...
                    if (dictionary.TryUpdate(key, newValue, value))
                        return Enums.DictionaryResult.ValueChanged
                            | Enums.DictionaryResult.KeyExists;
                }
            }
            return default;
        }
