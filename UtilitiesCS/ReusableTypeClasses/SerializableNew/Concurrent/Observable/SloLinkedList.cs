...
{
    public class SloLinkedList<T>
        : LockingObservableLinkedList<T>,
            ISmartSerializable<SloLinkedList<T>>
    {
        ...
        public SloLinkedList()
            : base()
        {
            ism = new SmartSerializable<SloLinkedList<T>>(this);
        }

        public SloLinkedList(IEnumerable<T> collection)
            : base(collection)
        {
            ism = new SmartSerializable<SloLinkedList<T>>(this);
        }
        ...
        public SloLinkedList<T> Deserialize(string fileName, string folderPath) =>
            ism.Deserialize(fileName, folderPath);

        public SloLinkedList<T> Deserialize(
            string fileName,
            string folderPath,
            bool askUserOnError
        ...
        public async Task<SloLinkedList<T>> DeserializeAsync<U>(SmartSerializable<U> config)
            where U : class, ISmartSerializable<U>, new() => await ism.DeserializeAsync(config);

        public async Task<SloLinkedList<T>> DeserializeAsync<U>(
        ...
        SloLinkedList<T> ISmartSerializable<SloLinkedList<T>>.Deserialize<U>(
        ...
        SloLinkedList<T> ISmartSerializable<SloLinkedList<T>>.Deserialize<U>(
            SmartSerializable<U> loader,
            bool askUserOnError,
            Func<SloLinkedList<T>> altLoader
        )
        {
            throw new NotImplementedException();
        }

        Task<SloLinkedList<T>> ISmartSerializable<SloLinkedList<T>>.DeserializeAsync<U>(
            SmartSerializable<U> config,
            bool askUserOnError,
            Func<SloLinkedList<T>> altLoader
        )
        ...
        public static class Static
        {
            ...
            public static SloLinkedList<T> Deserialize(
                string fileName,
                string folderPath,
                bool askUserOnError
            ...
            public static async Task<SloLinkedList<T>> DeserializeAsync<U>(
                SmartSerializable<U> config
            )
                where U : class, ISmartSerializable<U>, new() =>
