...
namespace UtilitiesCS.ReusableTypeClasses.Concurrent.Observable.Dictionary
{
        ...
        where TKey : notnull
    {
        ...
        {
            OnCollectionChanged(
                new DictionaryChangedEventArgs<TKey, TValue>(action, key, newValue, oldValue)
            );
        }
        ...
        public new void Clear()
        {
            base.Clear();
            OnCollectionChanged(
                new DictionaryChangedEventArgs<TKey, TValue>(NotifyCollectionChangedAction.Reset)
            );
        }

        public new TValue AddOrUpdate(
            TKey key,
            TValue addValue,
        ...
        )
        {
            ...
            foreach (var key in keys)
            {
                _observers.AddOrUpdate(
                    key,
                    new HashSet<IDictionaryObserver<TKey, TValue>> { observer },
                    (k, o) =>
                    {
                        o.Add(observer);
                        return o;
            ...
            return keys.ToDictionary(
                k => k,
                k => new HashSet<IDictionaryObserver<TKey, TValue>> { observer }
            );
        }
        ...
        {
            return AddPartialObserver(
                new SimpleActionDictionaryObserver<TKey, TValue>(action),
                keys
            );
        }
        ...
        public Dictionary<TKey, HashSet<IDictionaryObserver<TKey, TValue>>> RemoveAllObservers()
        {
            var ret = _observers.ToDictionary(
                kv => kv.Key,
                kv => new HashSet<IDictionaryObserver<TKey, TValue>>(kv.Value)
            );
            _observers.Clear();
            return ret;
        }
        ...
        private readonly ConcurrentDictionary<
            TKey,
            ICollection<IDictionaryObserver<TKey, TValue>>
        > _observers =
