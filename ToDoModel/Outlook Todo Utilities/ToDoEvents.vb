...
Public Module ToDoEvents
    Private Sub Debug_OutputNsStores(OlApp As Outlook.Application)
        Dim storeList As String = String.Empty

        Dim ns As Outlook.NameSpace = OlApp.Session
    ...
    Public Function GetListOfItemsInView_ToDo(OlApp As Outlook.Application) As List(Of Object)
        ...
        objView = OlApp.ActiveExplorer.CurrentView
        strFilter = "@SQL=" & objView.Filter

        OlItems = Nothing
        For Each oStore In OlApp.Session.Stores
    ...
    Public Function GetItemsInView_ToDo(OlApp As Outlook.Application) As Items
        Dim OlItems As Items
        Dim objView As View
        Dim OlFolder As Folder
        Dim strFilter As String
    ...
    Public Sub Refresh_ToDoID_Splits(OlApp As Outlook.Application)
        Dim objItem As Object
        Dim todo As ToDoItem
        Dim OlItems As Items = GetItemsInView_ToDo(OlApp)
        'QUESTION: Duplicate? If not, move to a class, module or library.
        For Each objItem In OlItems
    ...
    Private _blItemChangeRunning As Boolean = False

    Public Sub OlToDoItems_ItemChange(Item As Object,
                                      OlToDoItems As Outlook.Items,
        ...
        If _blItemChangeRunning = False Then
            ...
            Dim ProjInfo As IProjectInfo = AppGlobals.TD.ProjInfo
            Dim IDList As IListOfIDs = AppGlobals.TD.IDList
    ...
    Public Sub OlToDoItems_ItemAdd(Item As Object, AppGlobals As IApplicationGlobals)
        Dim todo As New ToDoItem(Item, OnDemand:=True)
        Dim ProjInfo As IProjectInfo = AppGlobals.TD.ProjInfo
        Dim IDList As IListOfIDs = AppGlobals.TD.IDList
    ...
    ''' </summary>
    Public Sub MigrateToDoIDs(OlApp As Application)
        'TODO: Move MigrateToDoIDs to a class, module, or library
        Dim ToDoItems As Outlook.Items = OlApp.GetNamespace("MAPI").GetDefaultFolder(OlDefaultFolders.olFolderToDo).Items
