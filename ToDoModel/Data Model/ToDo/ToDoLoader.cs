...
namespace ToDoModel.Data_Model.ToDo
{
    internal class ToDoLoader
    {
        ...
        internal System.Action OlSaver
        {
            get => _olSaver;
            set => _olSaver = value;
        }
        ...
        )
        {
            ...
            if (!_readonly)
            {
                if (objectSetter is null)
                {
                    throw new ArgumentNullException(
                        $"Method {nameof(SetAndSave)} failed because {nameof(objectSetter)} was passed as null"
                    );
                }
                objectSetter(value);
                if (objectSaver is not null)
                {
                    objectSaver();
                }
            }
        ...
        internal T GetOrLoad<T>(ref T value, Func<T> loader, params object[] dependencies)
        {
            if (value is null || value.Equals(default))
            {
                ...
                {
                    throw new ArgumentNullException(
                        $"Method {nameof(GetOrLoad)} failed the dependency check because {nameof(dependencies)} was passed as a null array"
                    );
                }
                if (dependencies.Any(x => x is null))
                {
                    var errors = dependencies
                        ...
                        .SentenceJoin();
                    throw new ArgumentNullException(
                        $"Method {nameof(GetOrLoad)} failed the dependency check because {nameof(dependencies)} contains a null value at position {errors}"
                    );
                }
            }
        ...
        )
        {
            if (
                ...
                && (dependencies is null || dependencies.Any(x => x is null))
            )
            {
                value = defaultValue;
                return value;
