...
// CTF_Inc(Inc_Num).Folderpath(j + 1) = CTF_Inc(Inc_Num).Folderpath(j)
// Next j
// CTF_Inc(Inc_Num).Email_Conversation_Count(i) = CTF_Map.Email_Conversation_Count
// CTF_Inc(Inc_Num).Folderpath(i) = CTF_Map.Folderpath
// added = True
// Exit For
// End If
// Next i
...
// CTF_Inc(Inc_Num).Folderpath(My.Settings.MaxFolders_ConvID) = CTF_Map.Folderpath
// End If
...
// CTF_Inc(Inc_Num).Folderpath(i) = "==============================="            'Set Folder name to lines so that they will not be accepted if they show up in selection list
// Next i
// End Sub
...
// CTF_Inc(Inc_Num).Folderpath(Inc_Position) = Map.Folderpath
// End Sub
...
// listOutput.Add(CTF_Inc(i).Folderpath(j))
