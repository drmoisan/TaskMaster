...
3) Ensure PR context artifacts exist and match the current branch state:
   ...
   - If missing OR clearly stale (e.g., branch head advanced, diff no longer matches working tree):
     - Run the repo tooling:
       - `VS Code command: `drm-copilot: Collect PR Context` (command ID: `drmCopilotExtension.collectPrContext`) --base ${input:PRBaseBranch}`
     - If that exact invocation is not runnable directly:
       - Use repo policy to choose the correct equivalent (e.g., `VS Code command: `drm-copilot: Collect PR Context` (command ID: `drmCopilotExtension.collectPrContext`) --base ...`).
